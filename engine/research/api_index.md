...
- `IggyQtShellWindow`: Qt host shell window that projects scene/ui models into desktop chrome, activity rail, workspace, panels, tool belt, palette, settings, and status bar widgets. In product play sessions it stores load/loop/play-mode build results and current play-mode state for stable product play context pointers, exposes a ready-state-only `Product Input Focus` View-menu toggle that updates only durable current focus via `RuntimeGameplayProductPlayMode::withInputFocus`, clears latest product play frame to null, and refreshes the read-only product play panel. Ready, focused product play sessions also map supported Qt key press/release events to an app-shell-owned transient `RuntimeGameplayProductInputFrame2D` containing only the latest product input event; Arrow/WASD map to movement, `E`/Return/Enter to interact, `I` to inspect, Space to wait, and Escape to cancel. The ready-state `Product Step` View action builds a local request-frame copy, enriches only that copy with `RuntimeGameplayProductInputContext` current-player-tile context, builds `RuntimeGameplayProductFrameRequestInput` from current `productPlayState_`, the enriched local frame, and shell-owned presentation camera config, calls `RuntimeGameplayProductFrameRequest::run` exactly once, replaces only transient app-shell play state/latest frame/presentation camera, clears stored transient product input after executed requests, and refreshes the product play panel; if Step is unavailable, input is not silently cleared. The shell stores no raw `QKeyEvent` objects or pointers, does not persist projected binding context back into `productInputFrame_`, and does not add automatic frame pumping, mouse screen-to-world/tile mapping, target discovery/search, player/modern NPC render expansion, settings persistence, keyboard shortcuts, save/load, or gameplay semantics.
