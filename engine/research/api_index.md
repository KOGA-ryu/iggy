...
- `RuntimeGameplayProductLoop`: product-owned loop state and one-frame step over successful product scenario loader output. It copies identity/package metadata, definition, lowered scenario, initial/current state, and next frame index; each step consumes one lowered frame, replaces scripted player frame intents with caller-provided `PlayerInputIntent2D`, delegates to `RuntimeGameplayOrchestratedFrameStep`, and carries state forward without TOML/package facade calls, file IO, raw device input mapping, UI/CLI coupling, presentation/camera ownership, save/load productization, pause/retry/reset UX, completion/failure semantics, package scanning, source mutation, or new gameplay semantics.
