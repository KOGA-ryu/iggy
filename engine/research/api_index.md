...
- `IggyQtShellWindow`: Qt host shell window that projects scene/ui models into desktop chrome, activity rail, workspace, panels, tool belt, palette, settings, and status bar widgets. In product play sessions it stores load/loop/play-mode build results and current play-mode state for stable product play context pointers, and exposes a ready-state-only `Product Input Focus` View-menu toggle that updates only durable current focus via `RuntimeGameplayProductPlayMode::withInputFocus`, clears latest product play frame to null, and refreshes the read-only product play panel without frame stepping, raw input routing, camera/render config, settings persistence, or gameplay changes.
...
- `iggy_qt_shell --play PATH`: explicit-path Qt product play launch/load/build consumer. It runs `RuntimeGameplayProductScenarioLoader::load(path)`, `RuntimeGameplayProductLoop::build(load)`, and `RuntimeGameplayProductPlayMode::build(loopBuild)`, stores load/loop/play-mode build results and play-mode state in `IggyQtShellWindow` for stable product play context pointers, reveals existing read-only `panel:product_play`, and leaves latest product play frame null; bad filesystem paths still open the shell with failed load/build state. `--play` and `--preview` are mutually exclusive and exit 2, and missing `--play` path exits 2. It does not call `RuntimeGameplayProductPlayMode::frame(...)`, `RuntimeGameplayProductPlaySurfaceFrame::build(...)`, map raw Qt/device input, create product input events, create default camera/render config, create presentation frames, pump app ticks, persist raw input/camera/presentation/render-frame data, save/load, scan/watch/discover packages, mutate sources, or add gameplay semantics.
