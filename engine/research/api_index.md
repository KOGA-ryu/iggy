...
- `iggy_scenario_toml_runner [--trace] <path>`: tiny dev-harness executable that reads one TOML source-plan file, adapts it with default ASCII source-plan conversion, runs the profile scenario runner, and prints summary counts plus final ASCII rows; `--trace` also prints per-frame counts and ASCII rows. No directory scanning, UI, save/load, runtime autorun, or new gameplay semantics.
