...
- `RuntimeGameplayScenarioLedgerReporter`: compact scenario execution ledger over `RuntimeGameplayScenarioResult`, with per-frame rows, final-state counts, deterministic tooling events, and optional profile scenario build/validation diagnostics; projection only.
