...
- `RuntimeGameplayAsciiSourcePlanProfileScenarioConverter`: explicit promotion layer from validated `RuntimeGameplayAsciiSourcePlan` into `RuntimeGameplayProfileScenarioDefinition`; promotes safe terrain, annotated NPC actors, one player-start marker into runtime player state, default controls optionally replaced by no-frame authored NPC controls, frame-id authored NPC controls and player commands as explicit per-frame inputs, caller-supplied frame/profile defaults, and optionally role-tag-promoted region AI maps without TOML parsing, interaction marker promotion, broad frame script invention, or raw/policy gameplay integration.
