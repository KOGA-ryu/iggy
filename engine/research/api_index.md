...
- `UiProductPlayModePanelModel`: read-only scene/UI projection over supplied product play build/state/latest-frame pointers. It backs `feature:product_play` / `panel:product_play` rows with existing runtime result fields including build/loop status, identity paths, loaded/focus/frame index/scenario frame count, latest frame/surface status, ignored input count, adapter counts, binding counts, step status/frame/counts, and presentation/render counts; the workspace populates it only when product play context exists and otherwise emits no missing-context diagnostic. It does not execute frames, call product loader/file/package/TOML APIs, call `RuntimeGameplayProductPlayMode::frame`, `RuntimeGameplayProductPlaySurfaceFrame::build`, or product step/run functions, mutate product play/runtime/gameplay state, create default camera/render config, map raw Qt/device input, persist raw input/camera/presentation/render-frame data, add `--play`, Qt launch, app shell, CLI behavior, scene/UI-owned frame pumping, pause/retry/reset/completion/failure/save-load semantics, package scanning/watching/discovery, source mutation, or new gameplay semantics.
...
- `IggyQtShellWindow::runProductFrameRequestOnce`: shared Qt product one-frame path used by manual `Product Step` and `Product Frame Pump`. After accumulator `buildFrame(...)` and after storing the returned accumulator state, it calls `RuntimeGameplayProductInputFrameTargetContext {}.enrich(...)` with `productPlayState_`, the built frame, and default spatial/reach configs, then passes `enrichedFrame.frame` into `RuntimeGameplayProductFrameRequest`. `Unchanged` and `NoEligiblePrimaryTile` paths pass copied unchanged frames without branching. Qt does not store or expose the enrichment diagnostics, target query result, target-context result, primary tile index/tile, or reach result; input focus gating, mouse/key mapping, pump timing, manual Step availability, accumulator one-shot drain, held-control behavior, frame request/play-surface/runtime helper semantics, `PrimaryTile` semantics, and Qt/runtime API boundaries remain unchanged.
