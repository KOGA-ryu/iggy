...
- `RuntimeGameplayAsciiSourcePlanProfileScenarioConverter`: explicit promotion layer from validated `RuntimeGameplayAsciiSourcePlan` into `RuntimeGameplayProfileScenarioDefinition`; promotes safe terrain, annotated NPC actors, default controls optionally replaced by one-frame authored NPC controls, caller-supplied frame/profile defaults, and optionally role-tag-promoted region AI maps without TOML parsing, interaction marker promotion, frame script invention, or raw/policy gameplay integration.
