...
- `IggyQtShellWindow`: Qt host shell window that projects scene/ui models into desktop chrome, activity rail, workspace, panels, tool belt, palette, settings, and status bar widgets. In product play sessions it stores load/loop/play-mode build results and current play-mode state for stable product play context pointers, exposes a ready-state-only `Product Input Focus` View-menu toggle that updates only durable current focus via `RuntimeGameplayProductPlayMode::withInputFocus`, clears latest product play frame to null, and refreshes the read-only product play panel. Ready, focused product play sessions map supported Qt key press/release edges into app-shell-owned transient `RuntimeGameplayProductInputAccumulatorState`: Arrow/WASD maintain held movement controls, `E`/Return/Enter, `I`, Space, and Escape queue one-shot interact/inspect/wait/cancel events, and focus disable clears the accumulator. Product play sessions also create an inert `QFrame#productViewport` inside `QFrame#mainSlot` as a Qt-shell-owned future event/render target boundary; it is present even for failed/not-ready play sessions, uses zero-margin/zero-spacing layout, expanding size policy, local viewport styling only, and resets `productViewport_` to null on rebuilds where no viewport is created. Manual `Product Step` and `Product Frame Pump` timer ticks share one frame helper: build accumulator frame output with `RuntimeGameplayProductInputContext` current-player-tile context, store returned accumulator state, build `RuntimeGameplayProductFrameRequestInput` from current `productPlayState_`, accumulator output, and shell-owned presentation camera config, call `RuntimeGameplayProductFrameRequest::run` exactly once, replace only transient app-shell play state/latest frame/presentation camera/context pointer, preserve held movement, drain one-shots, and refresh the product play panel. `Product Frame Pump` is a checkable ready-state View action using a window-owned `QTimer` at 250 ms / 4 Hz; it is independent of product input focus, stops when product play becomes unavailable/not ready, and does not stop on `NoFrameAvailable`. Latest frame presentation commands may now include product actor debug/material quads through the existing frame request path, but Qt does not own render semantics. The shell stores no raw `QKeyEvent` objects or pointers and does not persist accumulator state, pump state, projected binding context, camera, latest frame, render commands, actor render config, viewport geometry, projected pointer data, or render-frame data into runtime/session/gameplay/product-loop/play-mode/snapshot/save/settings/scene-UI truth, add a Qt mouse consumer, install an event filter or mouse handler on `productViewport_`, call `RuntimeGameplayProductPointerProjection` from Qt, use `QMouseEvent`/`QPoint`/QWidget coordinates for product input, map mouse screen-to-world/tile coordinates, synthesize `PrimaryPoint`/`PrimaryTile`, draw render commands, perform target discovery/search, add textured sprite/animation/material policy, settings persistence, keyboard shortcuts, save/load, or runtime/product gameplay semantics.
