...
- `IggyQtShellWindow`: Qt host shell window that projects scene/ui models into desktop chrome, activity rail, workspace, panels, tool belt, palette, settings, and status bar widgets. In product play sessions it stores load/loop/play-mode build results and current play-mode state for stable product play context pointers, exposes a ready-state-only `Product Input Focus` View-menu toggle that updates only durable current focus via `RuntimeGameplayProductPlayMode::withInputFocus`, clears latest product play frame to null, and refreshes the read-only product play panel. Ready, focused product play sessions map supported Qt key press/release edges into app-shell-owned transient `RuntimeGameplayProductInputAccumulatorState`: Arrow/WASD maintain held movement controls, `E`/Return/Enter, `I`, Space, and Escape queue one-shot interact/inspect/wait/cancel events, and focus disable clears the accumulator. The ready-state `Product Step` View action builds accumulator frame output with `RuntimeGameplayProductInputContext` current-player-tile context, stores returned accumulator state before the frame request, builds `RuntimeGameplayProductFrameRequestInput` from current `productPlayState_`, accumulator output, and shell-owned presentation camera config, calls `RuntimeGameplayProductFrameRequest::run` exactly once, replaces only transient app-shell play state/latest frame/presentation camera, preserves held movement across steps, drains one-shots, and refreshes the product play panel; if Step is unavailable, accumulator state is not silently cleared. The shell stores no raw `QKeyEvent` objects or pointers and does not persist accumulator state or projected binding context into runtime/session/gameplay/product-loop/play-mode/snapshot/save/settings/scene-UI truth, add automatic frame pumping, mouse screen-to-world/tile mapping, target discovery/search, player/modern NPC render expansion, settings persistence, keyboard shortcuts, save/load, or gameplay semantics.
