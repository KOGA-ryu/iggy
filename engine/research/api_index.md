...
- `RuntimeGameplayAsciiSourcePlanProfileScenarioConverter`: explicit promotion layer from validated `RuntimeGameplayAsciiSourcePlan` into `RuntimeGameplayProfileScenarioDefinition`; promotes safe terrain, annotated NPC actors, one player-start marker into runtime player state, authored interaction targets/effects into runtime interaction state, default controls optionally replaced by no-frame authored NPC controls, frame-id authored NPC controls and player commands as explicit per-frame inputs, caller-supplied frame/profile defaults, and optionally role-tag-promoted region AI maps without TOML parsing, extra marker promotion, invented frame semantics, or raw/policy gameplay integration.
