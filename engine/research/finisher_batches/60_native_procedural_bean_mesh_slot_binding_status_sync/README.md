# 60 Native Procedural Bean Mesh Slot Binding Status Sync
...
non-cube bean mesh and bound it to the player model slot.

## Integrated Commit

- `8ee4f327 Add native procedural bean mesh`

## Integrated Surface

- `engine/apps/native_play/NativeStaticMeshAsset.hpp` adds
  `NativeBeanStaticMeshAsset()`.
- The bean is an in-memory procedural CPU mesh using the existing
...
- `engine/apps/native_play/NativeVulkanRenderer.cpp` creates a separate
  `playerMesh_` from that asset through the existing mesh resource upload path.
- `NativeVulkanModelSlot::Player` now binds to `playerMesh_`.
- `Floor`, `Wall`, and `NpcActor` stay registered to the cube fallback.
...
- `git merge-base --is-ancestor 8ee4f327 HEAD`
- `git diff --check`
