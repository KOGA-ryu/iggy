...
Exact purpose: declare player slot identity and binding data for local single-player, AI, and future multiplayer participants.
...
- slot 0 is default local player
- slot kind local, remote, ai, observer
- actor `EntityId` may be invalid until bound

## Semantics

- slot is not an input device
- slot identity is stable across save/load
- authority code uses slot data to admit commands
...
- `src/runtime/player/PlayerSlot.hpp` exists in `/Users/kogaryu/iggy3d`;
