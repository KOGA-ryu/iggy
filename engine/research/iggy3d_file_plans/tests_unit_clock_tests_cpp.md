...
Exact purpose: prove clock mode and fixed tick behavior behavior for the complete `iggy3d` runtime.
...
- normal advances
- slow advances with semantic mode
- pause blocks automatic tick
- step while paused advances exactly one tick
- resume restores previous run mode
...
- `tests/unit/clock_tests.cpp` exists in `/Users/kogaryu/iggy3d`;
