...
Exact purpose: declare legal camera transitions between realtime and tactical modes.
...
- normal -> previous realtime
- slow -> tactical overhead/orbit
- paused keeps current tactical/realtime view unless explicitly changed

## Semantics

- policy has no raw mouse input
- policy does not mutate world
- clock mode changes are the trigger for tactical camera changes
...
- `src/runtime/camera/CameraModePolicy.hpp` exists in `/Users/kogaryu/iggy3d`;
