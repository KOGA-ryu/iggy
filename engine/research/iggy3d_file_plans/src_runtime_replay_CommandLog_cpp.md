...
Exact purpose: implement deterministic command history for replay, diagnostics, save summaries, and multiplayer ordering.
...
- next sequence
- append rules
- last rejected command lookup for retry
...
- records accepted and rejected commands
- separates gameplay commands from app-only IO side effects if required
- stores baseline hash for replay comparison

## Semantics

- append order is deterministic and stable
- rejected records never execute
- retry stores its own command and references the rejected source
...
- O(1) append, O(command count) replay/summary scans.
