...
Exact purpose: declare runtime clock mode truth for realtime, slow-time tactical, paused, and single-step execution.
...
- modes normal, slow, paused
- step request while paused
- last transition reason if diagnostics need it

## Semantics

- app wall-clock is not stored here
- slow time is semantic runtime mode
- paused sessions mutate only on explicit step/reset/load commands
...
- `src/runtime/clock/ClockState.hpp` exists in `/Users/kogaryu/iggy3d`;
