...
Exact purpose: declare the complete per-entity runtime truth stored in `WorldState`.
...
- kinds include player, pickup, door, marker, npc
- active/persistent flags
...
- entity state is save truth
- projection derives scene items from entities
- systems mutate only fields they own through `WorldState` helpers
...
- `src/runtime/world/EntityState.hpp` exists in `/Users/kogaryu/iggy3d`;
