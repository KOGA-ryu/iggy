...
Exact purpose: implement deterministic clock decisions from clock state and session control commands.
...
- transition helpers for normal, slow, paused, resume previous mode, consume step

## Semantics

- no direct wall-clock reads
- session runner supplies elapsed host time if needed but fixed ticks remain deterministic
...
- `src/runtime/clock/Clock.cpp` exists in `/Users/kogaryu/iggy3d`;
