...
Exact purpose: implement deterministic player slot storage and actor lookup.
...
- slot vector
- lookup by slot id
- actor binding updates
- default local player creation
...
- small vector of slots
- no unordered map required for first build
- stable slot ordering for replay/multiplayer

## Semantics

- roster does not read input devices
- roster does not move actors
- invalid slot lookup returns status rather than crashing
...
- O(player count), expected tiny for demo and local multiplayer.
...
- `src/runtime/player/PlayerRoster.cpp` exists in `/Users/kogaryu/iggy3d`;
