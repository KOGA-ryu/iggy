...
Exact purpose: implement the authoritative entity collection and deterministic world mutation API.
...
- vector of `EntityState` in allocation order
- next entity id
- fixture baseline copy support for reset

## Semantics

- all gameplay systems mutate entities through this API
- no renderer ids live here
- entity iteration order is deterministic
- reset restores the baseline fixture world
...
- O(entity count) lookup in first build; future indexes must preserve deterministic iteration.
...
- `src/runtime/world/WorldState.cpp` exists in `/Users/kogaryu/iggy3d`;
