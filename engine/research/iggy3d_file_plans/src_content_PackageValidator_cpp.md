...
Exact purpose: validate loaded package data and scenario references before runtime session creation.
...
- duplicate id/name checks
- semantic content diagnostics
...
- validates manifest id/version
- scenario existence
- first-room required entity/objective definitions
- unique stable names

## Semantics

- validator never mutates runtime
- all validation failures are structured diagnostics
- tests assert both pass and fail cases
...
- O(package field count plus scenario reference count).
...
- `src/content/PackageValidator.cpp` exists in `/Users/kogaryu/iggy3d`;
