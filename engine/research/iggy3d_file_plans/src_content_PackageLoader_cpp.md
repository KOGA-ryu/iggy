...
Exact purpose: load package manifest text from disk into `PackageManifest` without creating runtime state.
...
- manifest file IO
- minimal deterministic parse rules
- parse diagnostics
...
- accepts `package.iggy3d.toml` subset used by fixtures
- normalizes relative scenario paths against package directory

## Semantics

- load is read-only with respect to runtime
- parse order does not affect semantic output
- unknown required fields fail validation rather than being silently ignored
...
- O(file bytes plus parsed fields).
...
- `src/content/PackageLoader.cpp` exists in `/Users/kogaryu/iggy3d`;
