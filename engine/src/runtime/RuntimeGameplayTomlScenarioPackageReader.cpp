...
void AddIssue(
	RuntimeGameplayTomlScenarioPackageReadResult &result,
...
bool ReadManifest(RuntimeGameplayTomlScenarioPackageReadResult &result)
{
...
bool RuntimeGameplayTomlScenarioPackageReadResult::ok() const
{
	return status == RuntimeGameplayTomlScenarioPackageReadStatus::Read;
}

RuntimeGameplayTomlScenarioPackageReadResult
RuntimeGameplayTomlScenarioPackageReader::read(
	const std::filesystem::path &path) const
{
	...
	if (std::filesystem::is_directory(path, error)) {
		...
		result.status =
			RuntimeGameplayTomlScenarioPackageReadStatus::PackagePathNotSupported;
		return result;
	}
	...
	if (!std::filesystem::exists(result.manifestPath, error)) {
		...
		result.status =
			RuntimeGameplayTomlScenarioPackageReadStatus::MissingManifest;
		return result;
	}

	if (!ReadManifest(result)) {
		result.status =
			RuntimeGameplayTomlScenarioPackageReadStatus::ManifestInvalid;
		return result;
	}

	result.mainScenarioPath =
		(result.packageRoot / result.manifest.main).lexically_normal();
	result.status = RuntimeGameplayTomlScenarioPackageReadStatus::Read;
	return result;
}
