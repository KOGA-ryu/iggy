# Wave 01: Calculus production — contract revision 2
...
### difference_quotient: Derivatives from a difference quotient

Chapter `topic_0060` / **Differentiation**. Degree-at-most-two real polynomials, integer coefficients -9..9, integer point -3..3. Reuse reviewed expansion-versus-differentiation logic, keep h nonzero before the limit, distinguish f(a) and f prime(a). Independent checks must compare actually distinct computational procedures.

### polynomial_rules: Polynomial derivative rules

Chapter `topic_0060` / **Differentiation**. Real polynomials of degree at most three with bounded integer coefficients and evaluation points. Define powers, coefficients, derivative function versus value, sum/constant/power rules and their domain. Independently recover the h coefficient from expanding the original polynomial at a+h; compare it with coefficient-by-degree differentiation. Reject accidental equality between intended and distractor values; no transcendental derivatives or symbolic free-form grading.

## Writable files and identities

Your source boundary is this folder only:
`/Users/kogaryu/iggy3d/paths/content/authoring/production/wave01/calculus/`.
...
Use `build/production/wave01/calculus/` for generated inputs and evidence;
temporary scratch may live there. The existing shared candidate gate also
writes immutable output under `build/parallel-authoring/calculus/`, which
is allowed. Read but never overwrite old candidates or active/exported packages.

Reserve **publishable** package IDs `prod01_calculus_FAMILY`, version 1.
Its single canonical reading ID is `prod01_calculus_FAMILY_r`, shared by
all three set checks and all 18 questions. Staging-only six-question packages
retain `prod01_calculus_FAMILY_SET`; their question IDs may keep that
...
an assignment with subject `calculus`, source folder, reserved package ID
...
Write `build/production/wave01/calculus/production.json` with exactly these
