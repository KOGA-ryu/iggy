# Wave 01: Linear Algebra production — contract revision 2
...
### row_operations: Complete and reversible row operations

Chapter `worked_matrix_practice` / **Worked matrix practice**. Two equations in x,y using existing bounded MATRIX_ROLE_CHECKERS where valid. Signed augmented entries, whole-row operations, inverse operations, first-error repair and original-system verification including fractional pairs. Explicitly preserve the order of x and y and every constant column.

### determinants_2x2: Two by two determinants and invertibility

Chapter `topic_0093` / **Determinants**. Bounded real integer 2x2 matrices. Define positions and determinant ad-bc, sign under a row swap, zero versus nonzero determinant, and invertibility. Cover singular and nonsingular cases deliberately without confusing a singular coefficient matrix with a unique system outcome. Independently verify a nonzero inverse by exact matrix multiplication; check singularity via dependence/null-vector evidence. No arbitrary-size determinants or new solver runtime.

## Writable files and identities

Your source boundary is this folder only:
`/Users/kogaryu/iggy3d/paths/content/authoring/production/wave01/linear_algebra/`.
...
Use `build/production/wave01/linear_algebra/` for generated inputs and evidence;
temporary scratch may live there. The existing shared candidate gate also
writes immutable output under `build/parallel-authoring/linear_algebra/`, which
is allowed. Read but never overwrite old candidates or active/exported packages.

Reserve **publishable** package IDs `prod01_linear_algebra_FAMILY`, version 1.
Its single canonical reading ID is `prod01_linear_algebra_FAMILY_r`, shared by
all three set checks and all 18 questions. Staging-only six-question packages
retain `prod01_linear_algebra_FAMILY_SET`; their question IDs may keep that
...
an assignment with subject `linear_algebra`, source folder, reserved package ID
...
Write `build/production/wave01/linear_algebra/production.json` with exactly these
