# Wave 01: Trigonometry production — contract revision 2
...
### sine_turn: Sine equations in one turn

Chapter `topic_0032` / **Unit Circle Framework**. Real angles in radians on [0,2pi). Bounded A*sin(theta)+B=C with nonzero integer A, exact heights 0, +/-1/2 and +/-1 only. Prove complete solution counts including extreme heights and zero, check endpoints, forbid duplicate angles. Vary the six role decisions meaningfully; do not merely relabel the fixed pilot.

### cosine_turn: Cosine equations in one turn

Chapter `topic_0032` / **Unit Circle Framework**. Same bounded [0,2pi) framework for A*cos(theta)+B=C. Define cosine as horizontal coordinate and its reflection separately. Use exact heights 0, +/-1/2 and +/-1; verify each angle against cosine, correct branch count, and endpoint inclusion. Do not assume sine reflections apply unchanged.

## Writable files and identities

Your source boundary is this folder only:
`/Users/kogaryu/iggy3d/paths/content/authoring/production/wave01/trigonometry/`.
...
Use `build/production/wave01/trigonometry/` for generated inputs and evidence;
temporary scratch may live there. The existing shared candidate gate also
writes immutable output under `build/parallel-authoring/trigonometry/`, which
is allowed. Read but never overwrite old candidates or active/exported packages.

Reserve **publishable** package IDs `prod01_trigonometry_FAMILY`, version 1.
Its single canonical reading ID is `prod01_trigonometry_FAMILY_r`, shared by
all three set checks and all 18 questions. Staging-only six-question packages
retain `prod01_trigonometry_FAMILY_SET`; their question IDs may keep that
...
an assignment with subject `trigonometry`, source folder, reserved package ID
...
Write `build/production/wave01/trigonometry/production.json` with exactly these
