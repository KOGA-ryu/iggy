# Historical Paths workstream checkpoints

This is a retained evidence record, not the active plan or instructions.
Use [WORKSTREAMS.md](WORKSTREAMS.md) for current work and
[CONTENT_PRODUCTION.md](CONTENT_PRODUCTION.md) for the current production
contract. Old pilot scopes, pending-user checks and first-wave package counts
below describe their original checkpoints and are superseded where stated.

## Production Wave 01: four Terra writers active

The user authorized production and removed routine per-question user QA on
2026-09-10. The [production plan](CONTENT_PRODUCTION.md) now governs the active
textbook work. Four existing writer tasks, **algebra**, **trig**, **calc** and
**linear**, were dispatched on Terra High and observed active. Their exact
[assignments](../content/authoring/production/wave01/assignments.json) reserve
144 questions and 24 readings across eight topic families. Each writer owns
one separate source folder and returns six checked candidate packages.

The coordinator owns routine mathematical/prose review, integration, failed-case
corrections and serial publication. The user reviews product direction and
optional milestone demonstrations, not every new question. Existing pilot
acceptance language is historical and does not block this authorized production.
Writers send a completion handoff here so coordination does not depend on
repeated user prompts. No scheduler or additional workers were created.

The existing compiler, native textbook, question model and exporter remain
canonical. No runtime or 3D asset edits accompany this production launch.
Chapter bindings and all 24 reserved package identities were checked; all 30
historical reference hashes remain intact. The baseline active Library still
has 413 questions and 959 readings. Production delivery/review/publication
remain pending; a dispatched writer is not a finished content package.

Evidence: `build/production/wave01/launch.json`. No screenshots, captures,
windows, font probes or ImGui contexts. Work remains uncommitted. Next action:
coordinator intake and review as writer receipts arrive, followed by combined
import/save checks and publication. No per-question user test request is needed.

## Previous checkpoint: four reviewed subject pilots in the local Library
...
## Production Wave 01 complete — 2026-09-10

All eight revision-2 family packages are published: 144 new questions and eight
canonical readings across algebra, trigonometry, calculus and linear algebra.
The active Library contains 557 questions, 967 readings and 1,278 sections in
39 documents (606,287 bytes). Exact release inventory: `build/production/wave01/completion.json`.

The final pass moved linear feedback into the existing Markdown roles, removed
its duplicate serializer and arbitrary numeric fillers, completed whole-row
and row-swap arithmetic, and checked actual Markdown edits through the compiler.
Trigonometry completed both six-role Markdown templates, removed unused partial
templates and Python prose routes, and corrected known-angle terminology and
negative reflection arithmetic. Independent exact-case reviews, final teaching
delta review, import/repeat/rejection, old-stamp/byte preservation, saved-progress
and actual published native-adapter checks pass. No new runtime source or C++
lines were added in this closeout. Changes remain uncommitted.

Published authoring sources are frozen. No screenshots, windows or font probes
were used; visual appearance and learner outcomes were not observed. The next
candidate is a reproducible funding demonstration, not yet started.

## Production Wave 02 complete — 2026-09-10

The user redirected the next capability from demonstration packaging to depth
and authorized the same four subject tasks to build. Four locally published
families now add **48 complete problems, 233 decisions, 466 wrong-choice
corrections and four canonical lessons**: full linear equations, complete
trigonometric solution sets, derivatives/tangents, and two-equation systems.
Each family contains 12 routes in introductory, practice and mixed groups.

Independent review checked all 699 options and 233 reached states, together
with definitions, rule conditions, explanations, feedback and separate worked
examples. Bounded calculus/algebra feedback and lesson wording were corrected;
trigonometry's count grammar was polished. Algebra's existing mathematical
certificate gained separate displayed-form checks on its existing arithmetic
AST after review demonstrated that affine equivalence alone could miss a
requested simplification. Nine reviewer probes reject and four valid equivalent
forms pass. All content corrections were checked as exact compiled deltas.

The canonical owners remain LearningDocuments, LayeredQuestionSession, the
native textbook and existing persistence/exporter. Four ordinary Markdown
chapters, four package metadata files, four finite case files and four subject
certificates were added. No runtime C++ files/lines, content generators,
parallel parser or renderer were added in this wave. No competing maintained
route was introduced; candidate snapshots are historical evidence.

The final Library contains **605 questions, 971 readings, 1,282 sections and
36 structured lessons**, in 43 documents totaling 834,240 bytes. All 557 prior
question stamps and all prior published bytes are unchanged. Actual compiled
routes, closed worked disclosures, real Markdown edits, serial import/repeat,
failed-import recovery, saved-progress upgrade and the published native adapter
pass. Exact inventory and evidence: `build/production/wave02/completion.json`.
Final generation: `871e522d710b477285b4c858d18524910cc109cd47928e9501c054e1291bd9b4`.

All four sources are frozen; changes remain uncommitted and the separate 3D
work is preserved. No screenshots, captures, windows or font/ImGui probes were
used. Appearance and learner outcomes remain unobserved. This completes four
bounded families, not all curriculum sections or generic four-level support.
The next candidate is continued chapter depth. No Wave 03 or demonstration
packaging is started by this checkpoint.

## Completed Wave 02: full solving routes

**Complete: Production Wave 02 — full solving routes.** The four existing
subject tasks delivered 48 full problems and four canonical lessons, all
independently reviewed and published in the local Library.

| Subject | Complete problems | Decisions | Scope |
| --- | ---: | ---: | --- |
| Algebra | 12 | 52 | Fractions, brackets, variables on both sides, unique/empty/all-real sets |
| Trigonometry | 12 | 66 | Complete interval and general solutions, frequency/phase, impossible cases |
| Calculus | 12 | 56 | Chain/product rules, derivative values and tangent lines |
| Linear algebra | 12 | 59 | Complete two-equation solves, row operations, inconsistent and free-variable systems |

Each family uses one ordinary editable Markdown chapter with one lesson and
12 linked multi-step questions. Definitions, rule conditions, concrete working,
wrong-choice explanations and original-problem checks use the established
native textbook and symbolic choices. The canonical owners remain
LearningDocuments, LayeredQuestionSession, the native textbook, persistence
and tools/export_learning.py. No alternate parser, generator or renderer was
added. Runtime C++ change for this wave: **zero files and zero lines**.

Independent mathematical review covered all 699 options and 233 reached states;
all 466 wrong choices pass the actual compiled route checks. Teaching and
separate worked examples were read. Bounded wording corrections are complete.
Algebra's existing certificate now checks explicitly requested displayed form
as well as affine truth; nine reviewer probes reject and four valid alternate
forms pass. Exact compiled deltas preserve all other mathematical fields.

The live Library contains **605 questions, 971 readings, 1,282 sections and
36 structured lessons** in 43 documents (834,240 bytes). All 557 earlier question
stamps and all earlier published bytes are unchanged. Import, repeated import,
rejected-import recovery, saved-progress upgrade and the pure native adapter
pass against the unchanged Release binaries. Sources are frozen and changes
remain uncommitted. Separate 3D work is preserved. No screenshots, captures,
windows or font/ImGui probes were used; appearance and learner outcomes remain
unobserved.

- [Completion inventory and evidence](../build/production/wave02/completion.json)
- [Current production plan and QA ownership](CONTENT_PRODUCTION.md)
- [Completed depth contract](../content/authoring/production/wave02/BRIEF.md)
- [Production source ownership](../content/authoring/production/AGENTS.md)
- [Historical checkpoints](WORKSTREAM_HISTORY.md)

Wave 01 remains a completed, frozen starter release: eight families, 144 new
questions and eight readings. Its build and release evidence remains under
build/production/wave01/. Earlier candidates and writer receipts describe
historical intake states; each subject's release.json and the wave completion
record establish what is published.

**Next candidate: continue curriculum depth in the next chapter families.**
This checkpoint completes four bounded families, not all curriculum sections.
Keep symbolic multiple choice, preserve published payloads and saves, and reuse
the current authoring/import path. Generic four-level support remains separate
unfinished work. The coordinator owns routine QA and consolidation; the user
supplies product direction and optional milestone feedback. No Wave 03 is
assigned, and demonstration packaging remains deferred.
