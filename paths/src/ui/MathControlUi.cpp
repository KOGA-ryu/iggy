...
MathControlEdit compactMathControl(const MathParameterSpec& p,std::string_view label,double value,MathControlRange range,bool allowReset){
  ...
  if(table()){
    ...
    if(!p.choices.empty()){int choice=static_cast<int>(value);ImGui::SetNextItemWidth(control);edit.changed=ImGui::Combo("##choice",&choice,p.choices.data());edit.value=choice;}
