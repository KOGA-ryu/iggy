...
namespace runtime_interactables_internal {

bool automaticSourceMode(CreativeRuntimeLogicSourceMode mode) noexcept {
  return isAutomaticSourceMode(mode);
}

bool surfaceOwnedByRoomMesh(const RoomSpatialSurface& surface,
                            std::string_view targetMesh) noexcept {
  return surfaceBelongsToRoomMesh(surface, targetMesh);
}

bool surfaceOwnedByTarget(
    const RoomSpatialSurface& surface,
    const CreativeRuntimeInteractableState& target) noexcept {
  return surfaceBelongsToTarget(surface, target);
}
