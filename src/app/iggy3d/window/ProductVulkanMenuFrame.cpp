...
std::uint32_t scaledExtent(float value, float scale, std::uint32_t limit) {
  ...
  return std::clamp(static_cast<std::uint32_t>(std::lround(value * scale)), 1U, limit);
}
...
RenderUiRect renderRectFor(const ProductUiPrimitive& primitive,
  ...
  rect.height = scaledExtent(primitive.rect.height, scaleY, request.drawableHeight);
  rect.r = color.r;
  rect.g = color.g;
  rect.b = color.b;
  rect.a = color.a;
...
void appendTextQuads(ProductVulkanMenuFrame& frame,
  ...
  const DebugHudLayoutResult layout = layoutDebugHudTextAt(
      primitive.text,
      scaledOffset(primitive.rect.x, scaleX),
      scaledOffset(primitive.rect.y, scaleY),
