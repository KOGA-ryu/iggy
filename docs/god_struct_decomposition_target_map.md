...
| **10** | **FrontendWindowShell** | `ProductAppWindowState::frontendShell` owns the scalar menu/status fields moved by E174 (`launchAction`, `launchStatus`, `packageLoadStatus`, `openingMenuVisible`, `menuTextDrawn`, `selectedRowDrawn`, `mouseMenuSelectUsed`, `gamepadMenuSelectUsed`, `selectedSettingsTab`, `menuRowCount`, `framesPresented`, `eventPollCount`, `status`), `startup` moved by E175, and `productVulkanMenu` moved by E176. `automationControl` is ruled app-global and is not part of this store. | **NO / NO** — plain owned lifecycle/telemetry. | **Split across FOUR** — `StartupProbeFields.cpp` + `StartupWorldBuildoutFields.cpp` + `FrontendSettingsWindowFields.cpp` + `TailFields.cpp` (head=startup, tail=status). | **DONE as E174-E176.** E174 created the shell and moved scalar/menu/status state; E175 moved `startup`; E176 moved `productVulkanMenu`. |
