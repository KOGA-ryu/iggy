...
| **7** | **InputDeviceStore** | `ProductAppWindowState::inputDevice` now owns the G1 device/action fields moved by E165: `gamepadAvailable`, `gamepadName`, `gamepadMapping`, `lastInputAction`, `lastInputAccepted`. Remaining deferred slices keep `mouseCapture`, `controllerModeToggle`, `controllerAction`, `interactionMode`, and `interactionModeHud` on the god-struct for now. `gamepadMenuSelectUsed` is excluded and belongs to **FrontendWindowShell #10**. **DELETES** top-level `inputOwner` (349) + `gameplayInputSuppressed` (326). | **NO / NO** — owned input-surface + device identity. | `FrontendSettingsWindowFields.cpp` + `FeedbackSurfaceAutomationVulkanFields.cpp` (lastInputAction/Accepted). **MISMATCH — split two ways.** | **G1 DONE as E165.** E166/E167 remain deferred for controller/capture and interaction-mode fields. Prior partial landing deleted `window.inputOwner` + `window.gameplayInputSuppressed`; `mouseCapture.inputOwner` is receipt-only (single reader `FrontendSettingsWindowFields.cpp:173`) and **stays**. |
