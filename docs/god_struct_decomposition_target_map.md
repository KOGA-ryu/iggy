...
| **7** | **InputDeviceStore** | `ProductAppWindowState::inputDevice` now owns the G1 device/action fields moved by E165 (`gamepadAvailable`, `gamepadName`, `gamepadMapping`, `lastInputAction`, `lastInputAccepted`) plus the G2 controller/capture fields moved by E166 (`mouseCapture`, `controllerModeToggle`, `controllerAction`). Remaining deferred slice keeps `interactionMode` and `interactionModeHud` on the god-struct for now. `gamepadMenuSelectUsed` is excluded and belongs to **FrontendWindowShell #10**. **DELETES** top-level `inputOwner` (349) + `gameplayInputSuppressed` (326). | **NO / NO** — owned input-surface + device identity. | `FrontendSettingsWindowFields.cpp` + `FeedbackSurfaceAutomationVulkanFields.cpp` (lastInputAction/Accepted). **MISMATCH — split two ways.** | **G1 DONE as E165; G2 DONE as E166.** E167 remains deferred for interaction-mode fields. Prior partial landing deleted `window.inputOwner` + `window.gameplayInputSuppressed`; `mouseCapture.inputOwner` stays inside `inputDevice.mouseCapture`. |
