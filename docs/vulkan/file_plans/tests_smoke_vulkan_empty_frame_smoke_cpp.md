...
Exact path: `tests/smoke/vulkan_empty_frame_smoke.cpp`

Purpose: Prove acquire, clear/dynamic-rendering command recording, submit, present, and receipt for an empty frame.

## Build Position

Packet order: 5 - Swapchain, render loop, command recording, sync
Owner module: `empty frame smoke`
...
This file owns:
- Prove acquire, clear/dynamic-rendering command recording, submit, present, and receipt for an empty frame.
...
- `vulkan_empty_frame_smoke executable`
...
receipt_version=1
repo=iggy3d
file_plan=tests/smoke/vulkan_empty_frame_smoke.cpp
packet_order=5
...
fallback_area=empty_frame_smoke
fallback_reason=
strict_vulkan=true|false
result=pass|fail|skip
reason_code=
...
Smoke purpose:
- Prove the first end-to-end Vulkan frame path: device, surface, swapchain, frame sync, command recording, submit, present, validation counters, and receipt.
- The frame is clear-only. It does not prove scene rendering, shaders, descriptors, meshes, materials, or first-room visibility.
...
IGGY3D_EMPTY_FRAME_COUNT=<integer>
IGGY3D_EMPTY_FRAME_CLEAR=<r,g,b,a>
IGGY3D_DIAGNOSTICS_DIR=<path>
```

Required behavior:
1. Create Packet 4 Vulkan bootstrap state.
2. Create swapchain.
3. Create frame sync objects.
4. Create command recording pool and command buffers.
5. For each requested frame, acquire image, record clear-only dynamic rendering commands, submit, and present.
6. Print per-run receipt and final counters.
7. Destroy command recording, sync objects, swapchain, and bootstrap state in safe order.

Receipt fields:
```text
smoke=vulkan_empty_frame
frame_count_requested=<integer>
frames_attempted=<integer>
...
submit_result=<vk-result-name-or-none>
...
Strict lane policy:
- Strict lane fails on missing required Vulkan support, validation error, sync validation error, command recording failure, submit failure, present failure, malformed receipt, or device loss.
- Non-strict lane may skip before renderer work begins when host Vulkan/display support is unavailable.
- Suboptimal present may pass when a frame presented and recreate was requested or diagnosed.

Completion proof:
- At least one frame presents.
- Command receipt shows the color-to-present layout path.
- Sync receipt shows binary WSI semaphores and signaled frame fence.
- Runtime hash/replay output is absent or unchanged; this smoke is renderer proof, not gameplay proof.
...
- File `tests/smoke/vulkan_empty_frame_smoke.cpp` has an implementation packet that follows this plan.
