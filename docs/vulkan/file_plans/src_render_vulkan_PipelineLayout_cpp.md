...
Exact path: `src/render/vulkan/PipelineLayout.cpp`

Purpose: Implement first-room push-constant-only pipeline layout and future descriptor-compatible layout receipts.

## Build Position

Packet order: 6 - Shaders, pipeline, resources, descriptors
Owner module: `pipeline layout`
File kind: `source`
...
This file owns:
- Implement first-room push-constant-only pipeline layout and future descriptor-compatible layout receipts.
...
- `PipelineLayout::createFirstRoomLayout`
- `destroy`
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/PipelineLayout.cpp
packet_order=6
...
Create behavior:
- Validate descriptor set layout count against the selected `PipelineLayoutFamily`.
- Validate push constant ranges against shader interface policy.
- Create `VkPipelineLayout` once per pipeline family/variant.
- Name layout through debug utils when available.
- Emit deterministic layout receipt.

Packet 6 layout families:
```text
FirstRoomPushConstants:
...
MaterialUnlitTextured:
  ...
  blocked_until_reflection_verified=true
```
...
Compute cost:
- Layout creation happens during pipeline setup.
- No per-frame layout creation.
- Layout is recreated only when descriptor set layouts or push constant contract changes.

Verification:
- Pipeline smoke asserts first-room layout has zero descriptor set layouts.
- Descriptor smoke later asserts material layout compatibility after reflection gate passes.
...
- File `src/render/vulkan/PipelineLayout.cpp` has an implementation packet that follows this plan.
