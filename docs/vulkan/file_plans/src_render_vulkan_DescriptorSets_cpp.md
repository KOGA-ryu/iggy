...
Exact path: `src/render/vulkan/DescriptorSets.cpp`

Purpose: Implement descriptor set layout/pool/update policy for material and texture growth after first-room proof.

## Build Position

Packet order: 6 - Shaders, pipeline, resources, descriptors
Owner module: `descriptor sets`
File kind: `source`
...
This file owns:
- Implement descriptor set layout/pool/update policy for material and texture growth after first-room proof.
...
- `DescriptorSets::createMaterialLayout`
- `allocateMaterialSet`
- `updateMaterialSet`
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/DescriptorSets.cpp
packet_order=6
...
First-room behavior:
- Return an empty descriptor layout list for first-room pipeline creation.
- Do not allocate descriptor pools or sets for first-room vertex-color proof.
- Receipt must state descriptors are disabled for first-room pipeline.

Material behavior after gate:
- Create `descriptor_layout.material_unlit_textured.v1`.
- Create descriptor pool sized for first material smoke.
- Allocate material descriptor set.
- Write binding zero as combined image sampler with base-color image view and sampler.
- Validate layout compatibility with material pipeline layout.
...
descriptor_reflection_required
...
Receipt fields:
```text
descriptors_enabled=true|false
descriptor_set_layout_count=<integer>
descriptor_layout_names=<comma-separated-names>
descriptor_pool_count=<integer>
descriptor_set_count=<integer>
material_descriptor_count=<integer>
reflection_verified=true|false
```

Compute cost:
- First-room path has zero descriptor allocation cost.
- Material descriptor pools and sets are created outside normal per-frame command recording.
- Descriptor updates happen only when material GPU records are created or changed.

Verification:
- Descriptor smoke asserts reflection gate and binding metadata.
- Material smoke asserts descriptor set/image/sampler compatibility.
...
- File `src/render/vulkan/DescriptorSets.cpp` has an implementation packet that follows this plan.
