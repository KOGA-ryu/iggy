...
Exact path: `tests/smoke/vulkan_frame_hash_smoke.cpp`

Purpose: Prove normalized raw frame hash, tolerance metrics, and platform-specific baseline behavior.

## Build Position

Packet order: 7 - First visible room proof and packaging smoke
Owner module: `frame hash smoke`
...
This file owns:
- Prove normalized raw frame hash, tolerance metrics, and platform-specific baseline behavior.
...
- `vulkan_frame_hash_smoke executable`
...
receipt_version=1
repo=iggy3d
file_plan=tests/smoke/vulkan_frame_hash_smoke.cpp
packet_order=7
...
fallback_area=frame_hash_smoke
fallback_reason=
strict_vulkan=true|false
result=pass|fail|skip
reason_code=
...
Smoke purpose:
- Produce a normalized frame hash and tolerant visibility metrics for the first-room screenshot.
- Exact hashes are useful for local regressions; tolerant visibility is the cross-platform proof signal.

Hash input:
- Source pixels come from the screenshot smoke normalized RGBA buffer.
- Row pitch is removed before hashing.
- Channel order is normalized to RGBA.
- Alpha may be ignored for exact RGB hash when the screenshot policy says alpha is not stable.

Hash policy:
```text
frame_hash_algorithm=sha256_normalized_rgb
fixed_extent_required=true
exact_hash_cross_platform_strict=false
tolerant_visibility_cross_platform_strict=true
```

Visibility metrics:
```text
non_background_pixel_count=<integer>
non_background_pixel_coverage=<float>
room_proxy_pixel_count=<integer>
player_marker_pixel_count=<integer>
clear_color_only=true|false
```

Required receipt fields:
```text
smoke=vulkan_frame_hash
frame_hash_algorithm=sha256_normalized_rgb
frame_hash_exact=<hash-or-none>
frame_hash_path=<path-or-empty>
...
exact_hash_matched=true|false|unavailable
tolerant_visibility_passed=true|false
result=<pass|skip|fail>
reason_code=<stable-reason>
```

Skip/fail policy:
- Non-strict lane may skip before capture when screenshot artifact is unavailable.
- Strict frame-hash lane fails when no normalized screenshot exists, hash file cannot be written, image is clear-only, tolerant visibility fails, or receipt is malformed.
- Exact hash mismatch alone must not fail macOS/Linux/Windows cross-platform strict lanes when tolerant visibility passes.

Verification:
- Hash artifacts are written under `build/artifacts/render_diagnostics/frame_hashes/`.
- Runtime hash/replay result remains unchanged by visual hash calculation.
...
- File `tests/smoke/vulkan_frame_hash_smoke.cpp` has an implementation packet that follows this plan.
