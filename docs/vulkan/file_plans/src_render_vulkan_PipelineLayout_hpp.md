# src/render/vulkan/PipelineLayout.hpp
...
Exact path: `src/render/vulkan/PipelineLayout.hpp`

Purpose: Declare pipeline layout, push constant ranges, descriptor layout compatibility, and shader-interface hash policy.
...
This file owns:
- Declare pipeline layout, push constant ranges, descriptor layout compatibility, and shader-interface hash policy.
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/PipelineLayout.hpp
packet_order=6
...
enum class PipelineLayoutFamily {
    FirstRoomPushConstants,
...
struct PipelineLayoutCreateInfo {
    VkDevice device;
    PipelineLayoutFamily family;
    std::span<const VkDescriptorSetLayout> descriptorSetLayouts;
    std::span<const PushConstantRangeSpec> pushConstants;
...
RenderReceipt createPipelineLayout(const PipelineLayoutCreateInfo& createInfo, PipelineLayoutRecord* outRecord);
...
push_constant.0.name=clip_from_model
...
Ownership:
- Owns `VkPipelineLayout` and layout compatibility metadata.
- Does not own shader modules, graphics pipelines, descriptor pools, runtime material identities, or projection data.
- Runtime/projection never know pipeline layout names.

Verification:
- Shader interface tests prove push constant size/stage matches shader metadata.
- Pipeline smoke proves layout creation is validation-clean.
...
- File `src/render/vulkan/PipelineLayout.hpp` has an implementation packet that follows this plan.
