...
Exact path: `src/render/RenderDiagnostics.hpp`

Purpose: Declare backend-neutral diagnostics, receipt fields, renderer outcomes, and stable reason-code values.

## Build Position

Packet order: 1 - Backend-neutral renderer boundary
Owner module: `renderer diagnostics`
File kind: `header`
...
This file owns:
- Declare backend-neutral diagnostics, receipt fields, renderer outcomes, and stable reason-code values.
...
Normal path: format stable key-value receipts. Skip/fail: diagnostics record skip or fail decisions but do not make them; app/test harness maps outcomes to process exit.
...
receipt_version=1
repo=iggy3d
file_plan=src/render/RenderDiagnostics.hpp
packet_order=1
...
fallback_area=renderer_diagnostics
fallback_reason=
...
- File `src/render/RenderDiagnostics.hpp` has an implementation packet that follows this plan.
...
Header role: define the backend-neutral receipt vocabulary before Vulkan can emit device, swapchain, sync, or pipeline receipts.
...
platform_lane=moltenvk|native_vulkan|headless|software_vulkan
strict_vulkan=true|false
result=pass|fail|skip
reason_code=
message=
...
Rules:
- duplicate keys are allowed only when a file plan states a list field; Packet 1 should avoid duplicates.
- keys must be lowercase snake-case.
- values must be single-line.
- missing optional values use `unavailable`.
- diagnostics never expose raw Vulkan handles.
- diagnostics never contain structured object payloads.

Packet 1 acceptance:
```sh
ctest --test-dir build --output-on-failure -R 'render_diagnostics|render_reason_code'
rg -n '#include[ <"](SDL3/|SDL\.h|SDL_vulkan|vulkan/)|\bVk[A-Z][A-Za-z0-9_]*|\bVK_[A-Z0-9_]+' src/render/RenderDiagnostics.hpp
```
