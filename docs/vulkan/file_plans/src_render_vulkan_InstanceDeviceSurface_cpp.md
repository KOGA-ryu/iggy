...
Exact path: `src/render/vulkan/InstanceDeviceSurface.cpp`

Purpose: Implement instance creation, surface ownership handoff, physical-device selection, logical-device creation, queues, and diagnostics.

## Build Position

Packet order: 4 - Vulkan bootstrap, device, validation, diagnostics
Owner module: `instance device surface`
File kind: `source`
...
This file owns:
- Implement instance creation, surface ownership handoff, physical-device selection, logical-device creation, queues, and diagnostics.
...
When Vulkan is attempted on macOS, instance creation must enable
`VK_KHR_portability_enumeration` and set
`VK_INSTANCE_CREATE_ENUMERATE_PORTABILITY_BIT_KHR` before physical-device
enumeration when MoltenVK portability devices are expected. The dependency probe
records whether that extension, SDK root, ICD path, and MoltenVK evidence are
present before this module is required to create bootstrap resources.
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/InstanceDeviceSurface.cpp
packet_order=4
...
fallback_area=instance_device_surface
fallback_reason=
...
Startup sequence:
1. Collect platform-required instance extensions from `VulkanSurfaceProvider`.
2. Add renderer-required instance extensions for debug utils and portability when requested.
3. Enumerate requested validation layers through `DebugValidation`.
4. Create `VkInstance`.
5. Create debug messenger when validation/debug utils are enabled.
6. Create `VkSurfaceKHR` through the provider and transfer destruction responsibility to this owner.
7. Enumerate physical devices.
8. Query properties, features, extensions, queue families, surface present support, and required format support.
9. Reject candidates that fail hard gates.
10. Score remaining candidates deterministically.
11. Create logical device with unique selected queue families and enabled features/extensions.
12. Retrieve graphics and present queues.
13. Emit device diagnostics receipt.

Required first extensions and features:
- Instance: platform WSI extensions, `VK_EXT_debug_utils` when debug messenger is enabled, portability enumeration when required on Apple portability path.
- Device: `VK_KHR_swapchain`, portability subset when required, dynamic rendering path support.
- Features: dynamic rendering enabled for the selected path; timeline semaphore queried but disabled.

Failure behavior:
- No physical devices: `reason_code=no_physical_devices`.
- No candidate passes gates: `reason_code=no_suitable_physical_device`.
- Missing graphics queue: `reason_code=missing_graphics_queue`.
- Missing present queue: `reason_code=missing_present_queue`.
- Missing swapchain extension: `reason_code=missing_swapchain_extension`.
- Missing dynamic rendering support: `reason_code=dynamic_rendering_required_missing`.
- Logical device creation failure: `reason_code=logical_device_create_failed`.
- Queue retrieval failure: `reason_code=queue_retrieval_failed`.
...
device_vendor_id=<integer-or-none>
device_id=<integer-or-none>
...
enabled_features=<comma-separated-names>
missing_required_extensions=<comma-separated-names>
missing_required_features=<comma-separated-names>
...
Compute cost:
- Startup enumerates layers, extensions, physical devices, features, limits, queue families, and present support.
- Per-frame cost is zero.
- Full backend recreation repeats startup queries and must be diagnosed separately from normal resize.

Verification:
- `vulkan_device_smoke` validates required fields and clean teardown.
- `vulkan_platform_smoke` feeds the same platform extension and surface provider path.
- Include/firewall scans prove Vulkan object ownership stays under renderer Vulkan files and app WSI bridge files.
...
- File `src/render/vulkan/InstanceDeviceSurface.cpp` has an implementation packet that follows this plan.
