...
Exact path: `tests/unit/render_memory_budget_policy_tests.cpp`

Purpose: Prove allocation naming, VMA adoption gates, budget receipt fields, and no anonymous resource growth.

## Build Position

Packet order: 6 - Shaders, pipeline, resources, descriptors
Owner module: `memory tests`
File kind: `unit test`
...
This file owns:
- Prove allocation naming, VMA adoption gates, budget receipt fields, and no anonymous resource growth.
...
- `memory budget policy assertions`
...
receipt_version=1
repo=iggy3d
file_plan=tests/unit/render_memory_budget_policy_tests.cpp
packet_order=6
...
Test groups:
- `allocation_names_are_required`
- `first_room_allocation_names_are_stable`
- `budget_receipt_fields_are_present`
- `unnamed_allocations_fail_strict_policy`
- `per_frame_allocation_count_must_be_zero`
- `texture_work_requires_budget_diagnostics`
- `defragmentation_is_deferred`

Fixture rules:
- Use renderer memory policy tables and fake budget receipts.
- Do not require Vulkan loader, VMA library, display, SDL, or runtime session.
...
- Fails on missing allocation names, missing budget fields, or texture/material policy without diagnostics.
...
- File `tests/unit/render_memory_budget_policy_tests.cpp` has an implementation packet that follows this plan.
