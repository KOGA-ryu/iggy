...
Exact path: `shaders/vulkan/src/material_unlit_textured.frag.glsl`

Purpose: Define first unlit textured material fragment interface for sampled base-color texture output.
...
This file owns:
- Define first unlit textured material fragment interface for sampled base-color texture output.
...
- `set 1 binding 0 base_color_sampler`
...
receipt_version=1
repo=iggy3d
file_plan=shaders/vulkan/src/material_unlit_textured.frag.glsl
packet_order=6
...
fragment_input.0.name=in_uv0
descriptor_set.1.binding.0.name=base_color_texture
descriptor_set.1.binding.0.type=combined_image_sampler
descriptor_set.1.binding.0.stage=fragment
fragment_output.0.location=0
fragment_output.0.name=out_color
```

Gate:
- Material descriptor implementation requires reflection or generated manifest verification from generated SPIR-V.
- This shader must not be used to pass first-room proof.

Ownership:
- Owns unlit base-color texture sampling for renderer material path.
- Does not own color-space policy beyond consuming the renderer-selected sRGB texture view.
- Does not own material identity, fallback policy, asset validation, runtime truth, or descriptor allocation.

Build output:
```text
source=shaders/vulkan/src/material_unlit_textured.frag.glsl
artifact=build/generated/shaders/vulkan/<config>/material_unlit_textured.frag.spv
```

Verification:
- Descriptor smoke verifies set/binding/type/stage metadata before binding.
- Material smoke verifies sampled texture path and fallback material behavior.
...
- File `shaders/vulkan/src/material_unlit_textured.frag.glsl` has an implementation packet that follows this plan.
