...
Exact path: `src/render/vulkan/Swapchain.hpp`

Purpose: Declare swapchain lifecycle, selected format/color/present/extent state, image views, and recreate contract.
...
This file owns:
- Declare swapchain lifecycle, selected format/color/present/extent state, image views, and recreate contract.
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/Swapchain.hpp
packet_order=5
...
class Swapchain {
public:
    RenderReceipt create(const SwapchainCreateInfo& createInfo);
    RenderReceipt recreate(std::uint32_t drawableWidth, std::uint32_t drawableHeight);
    RenderReceipt destroy();
    SwapchainAcquireResult acquire(VkSemaphore imageAvailable);
    SwapchainPresentResult notePresentResult(VkResult presentResult);
    RenderReceipt markDrawableExtent(std::uint32_t drawableWidth, std::uint32_t drawableHeight);
...
Include policy:
- Vulkan headers are allowed because this file is under `src/render/vulkan/**`.
- Runtime, content, projection, save, SDL, and app platform headers are forbidden.
- Backend-neutral render public headers must not include this file.

Ownership:
- Owns `VkSwapchainKHR`, swapchain image handles borrowed from the swapchain, image views, selected format/color space/present mode, extent, image count, generation, dirty state, and recreate counters.
- Does not own `VkSurfaceKHR`, logical device, queues, command buffers, fences, semaphores, runtime state, projection outputs, or app event polling.

State rules:
- `Ready` is the only state where acquire/submit/present normal path may run.
- `NotDrawable` skips before acquire.
- `DirtyResize`, `DirtyOutOfDate`, and `DirtySuboptimal` request recreate at a safe point.
- `SurfaceLost`, `DeviceLost`, and `Failed` block normal frame rendering.

Receipt fields:
```text
swapchain_state=<state-name>
...
Verification:
- `vulkan_swapchain_smoke` proves create/destroy and selected metadata receipts.
- `vulkan_resize_minimize_smoke` proves dirty/not-drawable state transitions.
- Firewall scans prove no swapchain ownership leaks into runtime, content, projection, or save.
...
- File `src/render/vulkan/Swapchain.hpp` has an implementation packet that follows this plan.
