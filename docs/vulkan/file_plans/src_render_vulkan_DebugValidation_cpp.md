...
Exact path: `src/render/vulkan/DebugValidation.cpp`

Purpose: Implement validation layer discovery, debug utils messenger setup, strict failure policy, and message receipt routing.

## Build Position

Packet order: 4 - Vulkan bootstrap, device, validation, diagnostics
Owner module: `debug validation`
File kind: `source`
...
- `docs/vulkan/debug_validation.md`
- `docs/vulkan/vulkan_debug_labels_and_capture.md`

## Ownership

This file owns:
- Implement validation layer discovery, debug utils messenger setup, strict failure policy, and message receipt routing.
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/DebugValidation.cpp
packet_order=4
...
fallback_area=debug_validation
fallback_reason=
...
Plan sequence:
1. Read validation and sync-validation request modes from config.
2. Enumerate instance layers.
3. Detect `VK_LAYER_KHRONOS_validation`.
4. Request `VK_EXT_debug_utils` when debug messenger is enabled.
5. Configure sync validation through the supported validation-layer settings path for the active SDK.
6. Build instance layer and extension lists for `InstanceDeviceSurface`.
7. Emit a validation setup receipt.

Callback behavior:
- Count error, warning, info, and verbose severities.
- Count validation, performance, and general message types.
- Count sync validation hazards as sync validation messages.
- Store first validation error, first validation warning, and first sync validation error summaries.
- Append bounded raw message text to `validation.log` when diagnostics directory exists.
- Return `VK_FALSE` unless a later explicit debug-break policy changes callback behavior.

Strict behavior:
- Missing required validation layer fails with `reason_code=validation_layer_required_missing`.
- Missing required sync validation fails with `reason_code=sync_validation_required_missing`.
- Missing required debug utils fails with `reason_code=debug_utils_required_missing`.
- Debug messenger creation failure fails with `reason_code=debug_messenger_create_failed`.
- Any validation error in a strict validation lane fails the attempted smoke.
- Warnings are counted and logged in first Packet 4 strict lanes.
...
validation_verbose_count=<integer>
sync_validation_error_count=<integer>
sync_validation_warning_count=<integer>
first_validation_error=<summary-or-none>
first_validation_warning=<summary-or-none>
first_sync_validation_error=<summary-or-none>
validation_log_path=<path-or-empty>
```

Platform behavior:
- macOS SDK validation availability is reported honestly and does not replace native Linux/Windows validation lanes.
- Linux software Vulkan validation is diagnosed as software when used.
- Windows installed package smoke distinguishes missing loader, missing device, and missing validation layer.

Verification:
- Validation smoke asserts required missing-layer behavior through strict and non-strict lanes.
- Diagnostics smoke asserts counters and first-message fields are always present.
- Firewall scans prove callback code cannot call runtime/projection/save APIs.
...
- File `src/render/vulkan/DebugValidation.cpp` has an implementation packet that follows this plan.
