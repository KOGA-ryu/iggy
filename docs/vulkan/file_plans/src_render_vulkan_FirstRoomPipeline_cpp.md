...
Exact path: `src/render/vulkan/FirstRoomPipeline.cpp`

Purpose: Implement first-room shader modules, vertex input state, dynamic rendering pipeline, depth state, and pipeline diagnostics.

## Build Position

Packet order: 6 - Shaders, pipeline, resources, descriptors
Owner module: `first-room pipeline`
File kind: `source`
...
- `docs/vulkan/depth_and_coordinates.md`

## Ownership

This file owns:
- Implement first-room shader modules, vertex input state, dynamic rendering pipeline, depth state, and pipeline diagnostics.
...
- `FirstRoomPipeline::create`
- `FirstRoomPipeline::destroy`
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/FirstRoomPipeline.cpp
packet_order=6
...
fallback_area=first-room_pipeline
fallback_reason=
...
Create behavior:
- Use dynamic rendering pipeline creation metadata.
- Use selected swapchain color format and selected depth format.
- Use vertex input binding for `FirstRoomVertex_Pos3_Color3`.
- Use shader entry point `main` for vertex and fragment stages.
- Enable depth testing and depth writes for first-room geometry.
- Use backface culling for the first opaque room path.
- Use viewport/scissor as dynamic state when command recording sets them per frame.
- Disable blending for first-room opaque geometry.
- Do not create persistent pipeline cache for Packet 6.
...
shader_vertex_interface_mismatch
shader_fragment_interface_mismatch
...
pipeline_key=<stable-key>
pipeline_layout=push_constants_only
descriptor_set_layout_count=0
shader_vertex=first_room.vert.spv
shader_fragment=first_room.frag.spv
color_format=<format-name>
depth_format=<format-name>
pipeline_cache=disabled
pipeline_created=true|false
```

Compute cost:
- Pipeline creation happens outside normal frame recording.
- Extent-only resize does not recreate this pipeline.
- Color/depth format changes mark pipeline recreate required.

Verification:
- Pipeline smoke asserts pipeline metadata and zero validation errors.
- Vertex format tests assert C++ metadata matches shader locations.
...
- File `src/render/vulkan/FirstRoomPipeline.cpp` has an implementation packet that follows this plan.
