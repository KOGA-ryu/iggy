...
Exact path: `tests/unit/render_config_tests.cpp`

Purpose: Prove renderer config precedence, conflicts, strictness, and diagnostics path normalization.

## Build Position

Packet order: 1 - Backend-neutral renderer boundary
Owner module: `config tests`
File kind: `unit test`
...
This file owns:
- Prove renderer config precedence, conflicts, strictness, and diagnostics path normalization.
...
receipt_version=1
repo=iggy3d
file_plan=tests/unit/render_config_tests.cpp
packet_order=1
...
- File `tests/unit/render_config_tests.cpp` has an implementation packet that follows this plan.
...
Test role: prove renderer config is startup policy only and cannot leak into runtime truth.

Required test groups:
```text
default_config_resolves_to_null_renderer
vulkan_request_without_build_support_is_diagnosed
required_null_renderer_for_visual_proof_is_rejected
validation_required_without_vulkan_is_rejected
frames_in_flight_accepts_one_two_three
frames_in_flight_rejects_zero_and_large_values
present_mode_names_are_backend_neutral
shader_root_is_not_required_for_null_renderer
diagnostics_dir_is_output_only
```
...
- no Vulkan present-mode constants;
- no environment-dependent absolute paths in expected output;
- no save/replay/config serialization.

CTest labels:
```text
iggy3d;unit;render;config
```
