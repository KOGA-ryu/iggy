...
Exact path: `tests/smoke/vulkan_descriptor_smoke.cpp`

Purpose: Prove descriptor layout/pool/update path for material growth after first-room proof.

## Build Position

Packet order: 6 - Shaders, pipeline, resources, descriptors
Owner module: `descriptor smoke`
...
- `docs/vulkan/descriptor_policy.md`

## Ownership

This file owns:
- Prove descriptor layout/pool/update path for material growth after first-room proof.
...
- `vulkan_descriptor_smoke executable`
...
receipt_version=1
repo=iggy3d
file_plan=tests/smoke/vulkan_descriptor_smoke.cpp
packet_order=6
...
fallback_area=descriptor_smoke
fallback_reason=
strict_vulkan=true|false
result=pass|fail|skip
reason_code=
...
Smoke purpose:
- Prove descriptor layout/pool/set policy for the first material path after reflection gate passes.
- This smoke also verifies that first-room pipeline remains descriptor-free.
...
Required behavior:
- Verify first-room descriptor set count is zero.
- Verify material shader reflection or generated manifest for set one binding zero.
- Create material descriptor set layout.
- Create descriptor pool.
- Allocate one material descriptor set.
- Write combined image sampler descriptor using a valid test texture/sampler when material resource gate is open.

Receipt fields:
```text
smoke=vulkan_descriptor
first_room_descriptor_sets=0
material_descriptor_layout=descriptor_layout.material_unlit_textured.v1
descriptor_set_layout_count=<integer>
descriptor_pool_count=<integer>
descriptor_set_count=<integer>
set1_binding0_type=combined_image_sampler
reflection_verified=true|false
result=<pass|skip|fail>
reason_code=<stable-reason>
```

Skip/fail policy:
- Non-strict lane may skip descriptor material proof when reflection tooling is unavailable and material proof is not required.
- Strict descriptor lane fails when reflection is required and unavailable, descriptor metadata mismatches, descriptor allocation fails, descriptor write fails, or validation reports an error.

Verification:
- Runtime/content/projection/save scans contain no descriptor set or binding policy.
- Pipeline layout compatibility is asserted before command recording binds descriptor sets.
...
- File `tests/smoke/vulkan_descriptor_smoke.cpp` has an implementation packet that follows this plan.
