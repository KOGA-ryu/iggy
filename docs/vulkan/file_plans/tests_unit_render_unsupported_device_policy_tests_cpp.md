...
Exact path: `tests/unit/render_unsupported_device_policy_tests.cpp`

Purpose: Prove unsupported required gates map to skip or fail according to lane strictness and never mutate runtime state.

## Build Position

Packet order: 4 - Vulkan bootstrap, device, validation, diagnostics
Owner module: `unsupported tests`
File kind: `unit test`
...
This file owns:
- Prove unsupported required gates map to skip or fail according to lane strictness and never mutate runtime state.
...
- `unsupported-device policy assertions`
...
receipt_version=1
repo=iggy3d
file_plan=tests/unit/render_unsupported_device_policy_tests.cpp
packet_order=4
...
fallback_area=unsupported_tests
fallback_reason=
strict_vulkan=true|false
result=pass|fail|skip
reason_code=
...
Test groups:
- `no_physical_devices_is_unsupported_environment`
- `missing_graphics_queue_rejects_candidate`
- `missing_present_queue_rejects_surface_candidate`
- `missing_swapchain_extension_rejects_candidate`
- `missing_dynamic_rendering_rejects_candidate`
- `missing_depth_format_rejects_candidate_when_required`
- `software_device_rejected_when_not_allowed`
- `software_device_allowed_for_software_lane`
- `device_override_cannot_bypass_hard_gates`
- `scoring_is_deterministic_after_gates_pass`

Fixture rules:
- Use fake candidate records and fake feature reports.
- Do not create Vulkan objects.
- Do not require display, SDL, loader, ICD, validation layer, or runtime session.

Assertions:
- Hard-gate failure always beats score.
- Device override is honored only after hard gates pass.
- Missing support maps to `RenderOutcome::Unsupported`.
- Strict versus non-strict skip/fail behavior is not implemented inside device-selection policy.
- Software Vulkan acceptance is controlled by explicit config.
...
software_device_allowed=true|false
result=<pass|fail>
...
- Fails on nondeterministic scoring, gate bypass, or renderer-core strict/skip mapping.
...
- File `tests/unit/render_unsupported_device_policy_tests.cpp` has an implementation packet that follows this plan.
