...
Exact path: `tests/smoke/vulkan_device_lost_smoke.cpp`

Purpose: Prove device-loss diagnostics and shutdown path preserve runtime state and require restart when recovery is not supported.

## Build Position

Packet order: 7 - First visible room proof and packaging smoke
Owner module: `device-loss smoke`
...
- `docs/vulkan/vulkan_device_loss_recovery.md`

## Ownership

This file owns:
- Prove device-loss diagnostics and shutdown path preserve runtime state and require restart when recovery is not supported.
...
- `vulkan_device_lost_smoke executable`
...
receipt_version=1
repo=iggy3d
file_plan=tests/smoke/vulkan_device_lost_smoke.cpp
packet_order=7
...
fallback_area=device-loss_smoke
fallback_reason=
strict_vulkan=true|false
result=pass|fail|skip
reason_code=
...
Smoke purpose:
- Prove `VK_ERROR_DEVICE_LOST` maps to renderer failure handling, diagnostics, and shutdown without runtime mutation.
- The smoke should use a controlled injection path or result-mapping harness; it must not intentionally crash the GPU driver.

Required behavior:
1. Start from a valid Packet 4 or Packet 5 renderer setup.
2. Inject or observe a device-lost result at acquire, submit, present, fence wait, or allocation boundary.
3. Stop submitting new work.
4. Mark renderer device-lost state.
5. Emit device-lost receipt.
6. Attempt bounded shutdown according to device-loss policy.
7. Preserve runtime hash and replay state.

Required receipt fields:
```text
smoke=vulkan_device_lost
device_lost_detected=true|false
device_lost_source=<acquire|submit|present|fence_wait|allocation|injected>
first_lost_vk_result=VK_ERROR_DEVICE_LOST
device_name=<name-or-none>
driver_version=<value-or-none>
swapchain_recreate_attempted=false
new_submit_after_device_lost=false
shutdown_after_device_lost=true|false
...
Strict behavior:
- Device loss during an attempted visual frame exits nonzero in strict visual lanes.
- The smoke passes only when the expected device-lost failure path is observed and runtime invariance is preserved.
- The smoke fails if device loss triggers swapchain recreate, further submit, runtime mutation, missing receipt, or unbounded wait.

Artifact policy:
```text
artifact_root=build/artifacts/render_diagnostics/device_lost/
receipt=receipt.txt
validation_log=validation.log
device_receipt=device.txt
```

Verification:
- Unit result mapping already proves `VK_ERROR_DEVICE_LOST` maps to `device_lost`.
- This smoke proves high-level shutdown and artifact behavior.
...
- File `tests/smoke/vulkan_device_lost_smoke.cpp` has an implementation packet that follows this plan.
