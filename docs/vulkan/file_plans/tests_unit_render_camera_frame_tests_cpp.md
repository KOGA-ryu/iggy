...
Exact path: `tests/unit/render_camera_frame_tests.cpp`

Purpose: Prove RenderCameraFrame matrices, near/far values, and camera modes follow runtime-derived presentation rules.

## Build Position

Packet order: 1 - Backend-neutral renderer boundary
Owner module: `camera frame tests`
File kind: `unit test`
...
This file owns:
- Prove RenderCameraFrame matrices, near/far values, and camera modes follow runtime-derived presentation rules.
...
receipt_version=1
repo=iggy3d
file_plan=tests/unit/render_camera_frame_tests.cpp
packet_order=1
...
fallback_area=camera_frame_tests
fallback_reason=
strict_vulkan=true|false
result=pass|fail|skip
reason_code=
...
- File `tests/unit/render_camera_frame_tests.cpp` has an implementation packet that follows this plan.
...
Test role: prove renderer camera frame data is derived presentation data and is rejected when unsafe for drawing.

Required test groups:
```text
third_person_camera_frame_is_valid
first_person_camera_mode_is_named
tactical_overhead_camera_mode_is_named
near_plane_must_be_positive
far_plane_must_exceed_near_plane
non_finite_eye_is_rejected
non_finite_forward_is_rejected
non_finite_matrix_is_rejected
camera_mode_truth_is_not_mutated
```
...
Runtime boundary:
- this test may include public camera value types only when needed to build a derived `RenderCameraFrame`.
- it must not call camera mode mutation APIs.
- it must not simulate input events.
- it must not depend on swapchain extent or Vulkan clip-space implementation.

CTest labels:
```text
iggy3d;unit;render;camera
```

Packet 1 completion proof:
```sh
ctest --test-dir build --output-on-failure -R 'render_camera_frame'
```
