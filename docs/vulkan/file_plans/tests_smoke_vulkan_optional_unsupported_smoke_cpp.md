...
Exact path: `tests/smoke/vulkan_optional_unsupported_smoke.cpp`

Purpose: Prove unsupported required gates in optional Vulkan lane return skip with receipt.
...
This file owns:
- Prove unsupported required gates in optional Vulkan lane return skip with receipt.
...
- `vulkan_optional_unsupported_smoke executable`
...
receipt_version=1
repo=iggy3d
file_plan=tests/smoke/vulkan_optional_unsupported_smoke.cpp
packet_order=7
...
Smoke purpose:
- Prove non-strict Vulkan lanes report unsupported environments as diagnosed skips with exit code `77`.
- This smoke validates test-boundary behavior; renderer core still reports backend-neutral unsupported outcomes.

Injected unsupported cases:
```text
...
Required behavior:
- Stop before committed renderer work when the injected missing requirement is detected.
- Print receipt with `result=skip`.
- Use stable `reason_code`.
- Return process exit code `77`.
- Do not create runtime save/replay files.

Receipt fields:
```text
smoke=vulkan_optional_unsupported
strict_lane=false
unsupported_case=<case-name>
unsupported_policy=optional_skip
...
Failure conditions:
- Missing receipt.
- Exit code other than `77`.
- `result` not equal to `skip`.
- Skip after renderer submitted work.
- Runtime hash mutation.
- Unknown reason code.

Verification:
- CTest marks exit code `77` as skipped through `SKIP_RETURN_CODE`.
- Strict unsupported behavior is tested separately by `vulkan_strict_unsupported_smoke`.
...
- File `tests/smoke/vulkan_optional_unsupported_smoke.cpp` has an implementation packet that follows this plan.
