...
Exact path: `tests/smoke/vulkan_pipeline_smoke.cpp`

Purpose: Prove SPIR-V load, shader module creation, pipeline layout, dynamic rendering pipeline, and pipeline receipt.

## Build Position

Packet order: 6 - Shaders, pipeline, resources, descriptors
Owner module: `pipeline smoke`
...
This file owns:
- Prove SPIR-V load, shader module creation, pipeline layout, dynamic rendering pipeline, and pipeline receipt.
...
- `vulkan_pipeline_smoke executable`
...
receipt_version=1
repo=iggy3d
file_plan=tests/smoke/vulkan_pipeline_smoke.cpp
packet_order=6
...
Smoke purpose:
- Prove generated first-room SPIR-V can become shader modules, a push-constant-only pipeline layout, and one dynamic-rendering graphics pipeline.
- This smoke does not need to present a visible first room; Packet 7 owns visible proof.
...
Required behavior:
- Create Packet 4/5 Vulkan bootstrap and swapchain metadata.
- Load `first_room.vert.spv` and `first_room.frag.spv`.
- Create shader modules.
- Create first-room push-constant-only pipeline layout.
- Create first-room dynamic-rendering graphics pipeline using selected color/depth formats.
- Destroy pipeline, layout, and shader modules cleanly.

Receipt fields:
```text
smoke=vulkan_pipeline
...
shader_artifact_hashes=<hashes-or-none>
...
Skip/fail policy:
- Non-strict lane may skip before Vulkan work when host support is unavailable.
- Strict lane fails on missing shader artifacts, shader module creation failure, layout mismatch, pipeline creation failure, validation error, or malformed receipt.

Verification:
- Pipeline creation is outside normal per-frame work.
- Runtime hash/replay data is absent or unchanged.
...
- File `tests/smoke/vulkan_pipeline_smoke.cpp` has an implementation packet that follows this plan.
