# src/render/vulkan/FirstRoomPipeline.hpp
...
Exact path: `src/render/vulkan/FirstRoomPipeline.hpp`

Purpose: Declare first-room graphics pipeline, vertex format, dynamic rendering format dependencies, and draw proxy metadata.
...
This file owns:
- Declare first-room graphics pipeline, vertex format, dynamic rendering format dependencies, and draw proxy metadata.
...
- `class FirstRoomPipeline`
- `FirstRoomVertex`
- `FirstRoomPipelineCreateInfo`
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/FirstRoomPipeline.hpp
packet_order=6
...
RenderReceipt createFirstRoomPipeline(const FirstRoomPipelineCreateInfo& createInfo, FirstRoomPipelineRecord* outRecord);
...
Ownership:
- Owns one first-room `VkPipeline` and its diagnostic key.
- Does not own shader compilation, shader source, pipeline layout lifetime, swapchain lifetime, command recording state, mesh buffers, or runtime scene truth.

Verification:
- `vulkan_pipeline_smoke` proves creation and destruction.
- `vulkan_first_room_smoke` later proves the pipeline can draw visible room geometry.
...
- File `src/render/vulkan/FirstRoomPipeline.hpp` has an implementation packet that follows this plan.
