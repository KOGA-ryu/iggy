...
Exact path: `tests/unit/render_shader_interface_tests.cpp`

Purpose: Prove C++ vertex, push constant, descriptor, and shader-source interface declarations stay in sync.

## Build Position

Packet order: 6 - Shaders, pipeline, resources, descriptors
Owner module: `shader tests`
File kind: `unit test`
...
- `docs/vulkan/vulkan_spirv_reflection_policy.md`

## Ownership

This file owns:
- Prove C++ vertex, push constant, descriptor, and shader-source interface declarations stay in sync.
...
receipt_version=1
repo=iggy3d
file_plan=tests/unit/render_shader_interface_tests.cpp
packet_order=6
...
Test groups:
- `first_room_vertex_interface_matches_cpp_metadata`
- `first_room_fragment_interface_matches_cpp_metadata`
- `first_room_push_constant_is_64_bytes`
- `first_room_descriptor_count_is_zero`
- `material_unlit_textured_descriptor_binding_matches_policy`
- `material_descriptor_requires_reflection_gate`
- `shader_entry_points_are_main`
- `shader_interface_hash_is_stable`

Fixture rules:
- Use deterministic metadata from shader policy files or compiled C++ interface tables.
- Tests do not need Vulkan loader, display, SDL, runtime session, or shader compiler.
- Tests may read generated reflection manifests only when the build generated them.

Assertions:
- First-room vertex locations are position at location zero and color at location one.
- First-room fragment output is location zero.
- First-room pipeline layout has zero descriptor set layouts.
- Material shader descriptor set one binding zero is combined image sampler and fragment-visible.
- Runtime, content, projection, and save files contain no shader descriptor ownership.

Receipt assertions:
```text
pipeline_family=<first_room|material_unlit_textured>
shader_interface_hash=<stable-hash>
descriptor_sets=<count-or-list>
push_constant_bytes=<integer>
result=<pass|fail>
...
- Runs on every platform without graphics dependencies.
- Fails on interface drift, descriptor leakage, or missing owner metadata.
...
- File `tests/unit/render_shader_interface_tests.cpp` has an implementation packet that follows this plan.
