...
Exact path: `src/render/vulkan/RenderLoop.hpp`

Purpose: Declare per-frame acquire, record, submit, present, and recreate orchestration.
...
This file owns:
- Declare per-frame acquire, record, submit, present, and recreate orchestration.
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/RenderLoop.hpp
packet_order=5
...
struct RenderLoopCreateInfo {
    InstanceDeviceSurface* deviceSurface;
    Swapchain* swapchain;
    FrameSync* frameSync;
...
class RenderLoop {
public:
    RenderReceipt initialize(const RenderLoopCreateInfo& createInfo);
    VulkanFrameResult renderFrame(const FrameInput& frameInput);
    RenderReceipt resize(std::uint32_t drawableWidth, std::uint32_t drawableHeight);
    RenderReceipt waitIdle();
...
Include policy:
- Vulkan-private module headers are allowed.
- Runtime, content, projection, save, SDL, and app platform headers are forbidden.
- The loop accepts only backend-neutral `FrameInput`; it never reaches into runtime or projection internals.

Ownership:
- Owns frame ordering and coordination between swapchain, frame sync, command recording, validation counters, and diagnostics.
- Does not own Vulkan instance/device/surface, swapchain handle internals, sync object allocation, command pool allocation, runtime state, or projection production.

Packet 5 frame order:
1. Validate backend-ready state and `FrameInput`.
2. Skip before acquire when drawable extent is zero or swapchain state is `NotDrawable`.
3. Recreate swapchain at safe point when dirty and drawable extent is nonzero.
4. Wait current frame slot fence.
5. Acquire image.
6. Handle acquire result.
7. Record empty-frame command buffer for the acquired image.
8. Reset fence immediately before submit.
9. Submit graphics work.
10. Present submitted image.
11. Handle present result.
12. Emit frame receipt.
...
acquired_image_index=<integer-or-none>
...
Verification:
- `vulkan_empty_frame_smoke` proves one clear-only frame can acquire, record, submit, and present.
- `vulkan_resize_minimize_smoke` proves skip/recreate behavior stays outside runtime truth.
...
- File `src/render/vulkan/RenderLoop.hpp` has an implementation packet that follows this plan.
