...
Exact path: `src/render/null/NullRenderer.hpp`

Purpose: Declare the no-op backend used to prove render boundary and runtime invariance without graphics dependencies.
...
This file owns:
- Declare the no-op backend used to prove render boundary and runtime invariance without graphics dependencies.
...
receipt_version=1
repo=iggy3d
file_plan=src/render/null/NullRenderer.hpp
packet_order=2
...
- File `src/render/null/NullRenderer.hpp` has an implementation packet that follows this plan.
...
last_source_tick
last_frame_index
last_scene_item_count
last_debug_item_count
last_viewport_width
...
Ownership rules:
- owns only counters, last-frame metadata, and backend-neutral diagnostics;
- borrows `FrameInput` only during `submitFrame`;
- stores counts and ids, not projection pointers;
- owns no GPU resources, no Vulkan handles, no SDL handles, no shader paths, no runtime sessions.
...
```text
backend=null
renderer_lifecycle=not_initialized|ready|shutdown
submitted_frame_count=
accepted_frame_count=
rejected_frame_count=
last_source_tick=
...
rg -n '#include[ <"](SDL3/|SDL\.h|SDL_vulkan|vulkan/)|\bVk[A-Z][A-Za-z0-9_]*|\bVK_[A-Z0-9_]+' src/render/null/NullRenderer.hpp
```
