...
Exact path: `shaders/vulkan/src/material_unlit_textured.vert.glsl`

Purpose: Define first unlit textured material vertex interface for post-first-room material growth.
...
This file owns:
- Define first unlit textured material vertex interface for post-first-room material growth.
...
receipt_version=1
repo=iggy3d
file_plan=shaders/vulkan/src/material_unlit_textured.vert.glsl
packet_order=6
...
vertex_input.1.name=in_uv0
vertex_input.1.format=R32G32_SFLOAT
push_constant.0.name=clipFromModel
push_constant.0.offset=0
push_constant.0.size=64
fragment_varying.0.location=0
fragment_varying.0.name=out_uv0
```

Gate:
- This shader is planned for material smoke, not first-room proof.
- Descriptor/material path requires reflection or generated manifest verification before strict material smoke may pass.

Ownership:
- Owns material vertex transform and UV pass-through.
- Does not own material identity, texture upload, sampler policy, descriptor allocation, or runtime asset validation.

Build output:
```text
source=shaders/vulkan/src/material_unlit_textured.vert.glsl
artifact=build/generated/shaders/vulkan/<config>/material_unlit_textured.vert.spv
```

Verification:
- Shader build policy test proves artifact naming.
- Material smoke uses this shader only after descriptor/material gates are open.
...
- File `shaders/vulkan/src/material_unlit_textured.vert.glsl` has an implementation packet that follows this plan.
