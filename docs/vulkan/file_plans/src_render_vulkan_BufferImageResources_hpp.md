# src/render/vulkan/BufferImageResources.hpp
...
Exact path: `src/render/vulkan/BufferImageResources.hpp`

Purpose: Declare renderer-owned buffers, depth image, staging/readback buffers, allocation names, and VMA integration surface.
...
This file owns:
- Declare renderer-owned buffers, depth image, staging/readback buffers, allocation names, and VMA integration surface.
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/BufferImageResources.hpp
packet_order=6
...
Ownership:
- Owns GPU buffer/image records, allocation names, first-room geometry resources, depth resources, staging records, and texture records when texture path opens.
- Does not own content package truth, runtime inventory, projection semantics, descriptor set layouts, pipeline creation, or swapchain image handles.

Packet 6 resources:
- First-room vertex buffer.
- First-room index buffer when indexed drawing is used.
- Depth image and image view for current swapchain extent.
- Staging buffer helper for initial geometry upload.
- Texture/material resources remain gated until descriptor/reflection/material smoke contracts pass.
...
memory_budget_reported=true|false
```

Verification:
- `vulkan_memory_smoke` proves named first-room allocations and no per-frame allocation in steady state.
- Memory budget unit tests prove required receipt fields and naming policy.
...
- File `src/render/vulkan/BufferImageResources.hpp` has an implementation packet that follows this plan.
