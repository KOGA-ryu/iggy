...
Exact path: `tests/smoke/vulkan_validation_smoke.cpp`

Purpose: Prove validation layer, debug messenger, and sync validation request/require behavior.

## Build Position

Packet order: 4 - Vulkan bootstrap, device, validation, diagnostics
Owner module: `validation smoke`
...
This file owns:
- Prove validation layer, debug messenger, and sync validation request/require behavior.
...
- `vulkan_validation_smoke executable`
...
receipt_version=1
repo=iggy3d
file_plan=tests/smoke/vulkan_validation_smoke.cpp
packet_order=4
...
fallback_area=validation_smoke
fallback_reason=
strict_vulkan=true|false
result=pass|fail|skip
reason_code=
...
Smoke purpose:
- Prove validation layer discovery, validation mode handling, debug utils extension behavior, debug messenger creation/destruction, validation counters, and strict-lane failure rules.
- This smoke may reuse Packet 4 instance/device bootstrap but does not require swapchain presentation.
...
sync_validation_mode=off
sync_validation_mode=optional
sync_validation_mode=required
```

Skip/fail policy:
- Non-strict validation smoke may skip validation-specific proof when the validation layer is unavailable.
- Strict validation smoke fails when `VK_LAYER_KHRONOS_validation` is missing.
- Strict validation smoke fails when required debug utils functions are missing.
- Strict sync-validation smoke fails when sync validation is unavailable or reports an error.
- Any validation error emitted by the smoke path fails strict validation lanes.

Required receipt fields:
```text
smoke=vulkan_validation
...
Artifact rules:
- Write `receipt.txt` for every run.
- Write `validation.log` when validation callback is active.
- Log content may include raw validation text; receipt fields remain stable key-value text.

Verification:
- Test verifies debug messenger is destroyed before instance teardown.
- Test verifies callback counters are present even with zero messages.
- Test verifies runtime state is never touched by validation callback.
...
- File `tests/smoke/vulkan_validation_smoke.cpp` has an implementation packet that follows this plan.
