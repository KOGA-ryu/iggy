...
Exact path: `tests/smoke/vulkan_memory_smoke.cpp`

Purpose: Prove first-room buffers, depth resources, staging upload, allocation naming, and memory budget receipt.

## Build Position

Packet order: 6 - Shaders, pipeline, resources, descriptors
Owner module: `memory smoke`
...
This file owns:
- Prove first-room buffers, depth resources, staging upload, allocation naming, and memory budget receipt.
...
- `vulkan_memory_smoke executable`
...
receipt_version=1
repo=iggy3d
file_plan=tests/smoke/vulkan_memory_smoke.cpp
packet_order=6
...
Smoke purpose:
- Prove VMA allocator creation, first-room buffer/image allocations, staging upload, depth resource creation, allocation naming, and budget diagnostics.
- This smoke does not prove material texture sampling.
...
Required behavior:
- Create Packet 4 Vulkan bootstrap state.
- Create VMA allocator.
- Create first-room vertex/index resources through named allocations.
- Upload geometry through staging resource.
- Create depth image resource for a test extent.
- Query or report memory budget availability.
- Destroy resources before allocator/device teardown.
...
unnamed_allocation_count=0
allocation_names=<comma-separated-names>
vertex_buffer_count=<integer>
index_buffer_count=<integer>
depth_image_created=true|false
staging_upload_count=<integer>
...
Skip/fail policy:
- Non-strict lane may skip before allocator creation when Vulkan setup is unavailable.
- Strict lane fails on allocator failure, unnamed allocation, upload failure, depth resource failure, memory validation error, or malformed receipt.

Verification:
- Smoke proves resource teardown is clean.
- Memory budget policy tests cover schema without GPU dependency.
...
- File `tests/smoke/vulkan_memory_smoke.cpp` has an implementation packet that follows this plan.
