...
Exact path: `src/render/vulkan/ShaderModule.cpp`

Purpose: Implement SPIR-V file loading, module creation/destruction, stage receipt fields, and shader-missing error mapping.
...
This file owns:
- Implement SPIR-V file loading, module creation/destruction, stage receipt fields, and shader-missing error mapping.
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/ShaderModule.cpp
packet_order=6
...
Create sequence:
1. Resolve configured shader root and requested SPIR-V path.
2. Read SPIR-V bytes into an owned byte vector.
3. Reject empty file or byte size that is not four-byte aligned.
4. Compute artifact hash for diagnostics.
5. Fill `VkShaderModuleCreateInfo`.
6. Call `vkCreateShaderModule`.
7. Name the shader module through debug utils when available.
8. Return a `ShaderModuleRecord` and receipt.

Destroy sequence:
- Destroy module with `vkDestroyShaderModule` when handle is valid.
- Clear handle and artifact metadata after destruction.
- Do not destroy a shader module while a pipeline creation call is using it.
...
Compute cost:
- Startup/pipeline creation reads shader bytes and creates modules.
- No per-frame shader file reads.
- No runtime shader compilation.

Platform behavior:
- macOS/MoltenVK, Linux, and Windows consume the same SPIR-V artifact schema.
- Windows multi-config output is resolved through configured shader root, not current working directory.

Verification:
- Pipeline smoke fails strict lanes on missing first-room SPIR-V.
- Diagnostics smoke prints shader hash and stage fields.
...
- File `src/render/vulkan/ShaderModule.cpp` has an implementation packet that follows this plan.
