...
Exact path: `tests/unit/package_runtime_lookup_tests.cpp`

Purpose: Prove executable-relative, resource-root, shader-root, diagnostics-dir, and platform layout lookup order.

## Build Position

Packet order: 3 - Visual app and SDL platform shell
Owner module: `package lookup tests`
File kind: `unit test`
...
This file owns:
- Prove executable-relative, resource-root, shader-root, diagnostics-dir, and platform layout lookup order.
...
receipt_version=1
repo=iggy3d
file_plan=tests/unit/package_runtime_lookup_tests.cpp
packet_order=3
...
fallback_area=package_lookup_tests
fallback_reason=
strict_vulkan=true|false
result=pass|fail|skip
reason_code=
...
Test groups:
- `build_tree_shader_root_resolves`
- `installed_windows_shader_root_resolves`
- `installed_linux_share_root_resolves`
- `macos_bundle_resource_root_resolves`
- `explicit_shader_root_missing_fails`
- `diagnostics_dir_can_be_created`
- `diagnostics_dir_unwritable_fails`
- `current_working_directory_is_not_used`
- `headless_mode_allows_missing_shader_root`
- `required_graphics_runtime_fails_missing_resource_root`

Fixture rules:
- Use temp directories created by the test process.
- Use explicit executable path and root overrides so tests do not depend on the developer machine layout.
- Environment-variable tests must restore previous environment values before exiting.
- No Vulkan, SDL, display, shader compiler, or runtime session is required.

Assertions:
- Lookup source strings match the selected source exactly.
- Missing required roots return stable reason codes.
- Diagnostics directory is created only by diagnostics policy, not by resource or shader lookup.
- Changing current working directory does not affect any resolved path.
- Returned paths are absolute when present.

Receipt assertions:
```text
package_mode=<expected-mode>
resource_root_source=<expected-source>
shader_root_source=<expected-source>
diagnostics_dir_source=<expected-source>
result=<pass|fail>
...
- Pass on every platform without graphics dependencies.
- Fail on nondeterministic source selection, current-directory dependency, or missing stable reason.
...
- File `tests/unit/package_runtime_lookup_tests.cpp` has an implementation packet that follows this plan.
