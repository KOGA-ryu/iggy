...
Exact path: `src/render/vulkan/InstanceDeviceSurface.hpp`

Purpose: Declare Vulkan instance, surface, physical/logical device, queue, feature, and extension ownership.
...
This file owns:
- Declare Vulkan instance, surface, physical/logical device, queue, feature, and extension ownership.
...
- `InstanceDeviceSurfaceCreateInfo`
- `DeviceSelectionReceipt`
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/InstanceDeviceSurface.hpp
packet_order=4
...
Include policy:
- Vulkan headers are allowed here.
- Standard callback/value types are allowed.
- SDL and native window headers are forbidden; surface creation is supplied by `VulkanSurfaceProvider`.
- Runtime, content, projection, save, and backend-neutral render public headers must not include this file.

Ownership:
- Owns `VkInstance`, backend-owned `VkSurfaceKHR`, selected `VkPhysicalDevice` reference, `VkDevice`, selected queue handles, queue-family selection, enabled extension/feature records, and device diagnostics.
- Does not own SDL window lifetime, swapchain images, command buffers, pipelines, buffers, images, runtime truth, or projection output.

Lifetime:
- Instance is created before surface.
- Surface is created before present support queries.
- Physical device is borrowed from instance enumeration.
- Logical device is destroyed before surface and instance.
- Debug messenger lifetime is coordinated with `DebugValidation`.

Verification:
- `vulkan_device_smoke` creates and destroys this object with a real surface provider.
- Unit policy tests prove fake unsupported candidates map to the same reason codes as real startup.
...
- File `src/render/vulkan/InstanceDeviceSurface.hpp` has an implementation packet that follows this plan.
