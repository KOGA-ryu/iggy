...
Exact path: `src/render/vulkan/DebugValidation.hpp`

Purpose: Declare validation layer, debug messenger, sync validation, debug-name, and message-policy helpers.
...
- `docs/vulkan/vulkan_validation_message_policy.md`

## Ownership

This file owns:
- Declare validation layer, debug messenger, sync validation, debug-name, and message-policy helpers.
...
- `DebugValidationConfig`
- `DebugValidation`
- `ValidationMessagePolicy`
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/DebugValidation.hpp
packet_order=4
...
enum class ValidationRequestMode {
    Off,
    Optional,
    Required
};
...
struct DebugValidationConfig {
    ValidationRequestMode validationMode;
    ValidationRequestMode syncValidationMode;
    bool strictVulkan;
    bool debugMessengerRequested;
    std::filesystem::path diagnosticsDir;
};

struct DebugValidationPlan {
    bool validationLayerRequested;
    bool validationLayerRequired;
    bool validationLayerFound;
    bool debugUtilsRequested;
    bool debugUtilsRequired;
    bool debugUtilsFound;
    bool syncValidationRequested;
    bool syncValidationRequired;
...
struct DebugValidationCounters {
    ...
    std::uint64_t infoCount;
    std::uint64_t verboseCount;
    std::uint64_t syncErrorCount;
    std::uint64_t syncWarningCount;
};
...
Include policy:
- Vulkan headers and filesystem paths are allowed.
- Runtime, content, projection, save, SDL, and app platform headers are forbidden.
- This header remains private to Vulkan bootstrap and diagnostics tests.

Ownership:
- Owns validation mode translation, layer/extension planning, debug messenger handle, validation counters, and validation artifact routing.
- Does not own Vulkan instance creation, runtime logging policy, gameplay errors, or release crash policy.

Semantics:
- Required validation missing fails strict validation lanes.
- Non-required validation missing records `validation=unavailable`.
- Debug messenger is created after instance creation and destroyed before instance destruction.
- Validation callback records counts and bounded message summaries only.
- Validation callback must not call runtime, projection, save, renderer submit, or Vulkan queue operations.

Verification:
- `vulkan_validation_smoke` proves validation plan, debug messenger lifetime, and receipt fields.
- `vulkan_diagnostics_smoke` proves validation counters are present even when validation is unavailable.
...
- File `src/render/vulkan/DebugValidation.hpp` has an implementation packet that follows this plan.
