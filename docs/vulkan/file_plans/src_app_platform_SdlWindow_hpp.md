...
Exact path: `src/app/platform/SdlWindow.hpp`

Purpose: Declare SDL window ownership, event polling, drawable-size query, and platform receipt surface.
...
This file owns:
- Declare SDL window ownership, event polling, drawable-size query, and platform receipt surface.
...
receipt_version=1
repo=iggy3d
file_plan=src/app/platform/SdlWindow.hpp
packet_order=3
...
Include policy:
- This header may include SDL declarations because it is under `src/app/platform/**` and is not a runtime, content, projection, or save header.
- Prefer a forward declaration of `SDL_Window` when the SDL version permits it.
- This header must not include Vulkan headers.
- Runtime, content, projection, save, and core gameplay headers must never include this header.

Ownership:
- Owns one SDL window wrapper and the event-state snapshot for the visual app shell.
- Does not own `VkSurfaceKHR`, Vulkan instance/device/swapchain, renderer backend objects, runtime state, runtime commands, or projection outputs.
- The app owns the `SdlWindow` object; the object owns the native window lifetime.

Semantics:
- `pollEvents()` updates only `SdlWindowEventState`.
- `isDrawable()` is false when the drawable extent is zero or when the window is minimized.
- Resize/minimize events become app-visible state; they do not directly recreate a swapchain.
- `nativeWindow()` is exposed only for platform bridge files and smoke tests.

Reason fields for callers:
- `sdl_window_ok`
- `sdl_video_init_failed`
- `sdl_window_create_failed`
- `sdl_window_not_drawable`

Compute cost:
- Initialization creates the native SDL window and queries drawable size once.
- Per-frame cost is one SDL event pump plus one drawable extent query.
- Resize cost is event-state update only; Vulkan swapchain work is owned elsewhere.

Verification:
- Unit coverage is through compile and API-shape checks when available.
- `vulkan_platform_smoke` proves construction, event polling, drawable extent reporting, and clean destruction.
- Boundary scans prove SDL wrapper headers do not leak into runtime ownership surfaces.
...
- File `src/app/platform/SdlWindow.hpp` has an implementation packet that follows this plan.
