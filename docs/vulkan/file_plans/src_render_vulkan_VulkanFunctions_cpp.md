...
Exact path: `src/render/vulkan/VulkanFunctions.cpp`

Purpose: Implement vkGetInstanceProcAddr and vkGetDeviceProcAddr loading with core/KHR name policy diagnostics.
...
This file owns:
- Implement vkGetInstanceProcAddr and vkGetDeviceProcAddr loading with core/KHR name policy diagnostics.
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/VulkanFunctions.cpp
packet_order=4
...
Load sequence:
1. Confirm loader strategy.
2. Use normal prototypes for core calls when linked loader is active.
3. Load instance extension functions through `vkGetInstanceProcAddr`.
4. Load debug utils functions only when requested and enabled.
5. Load device extension functions through `vkGetDeviceProcAddr` after logical device creation.
6. Load dynamic rendering functions for the selected source.
7. Validate active function table completeness.
8. Emit a deterministic receipt.

Failure behavior:
- Missing required instance function returns `reason_code=instance_function_missing`.
- Missing required device function returns `reason_code=device_function_missing`.
- Missing debug utils function fails only when debug utils is required.
- Missing swapchain functions after enabling `VK_KHR_swapchain` fails Vulkan startup.
- Core/KHR dynamic rendering mismatch fails with `reason_code=dynamic_rendering_core_khr_mismatch`.

Lifetime rules:
- Instance function table is invalid before instance creation and after instance destruction.
- Device function table is invalid before logical device creation and after logical device destruction.
- Function table state must be cleared during backend teardown.
- Device loss does not mutate runtime truth, but backend teardown invalidates function use.

Platform behavior:
- macOS, Linux, and Windows use the same receipt schema.
- MoltenVK portability affects extension/feature selection, not Apple-only function calls.
- Manual loader opening is disabled in first Packet 4 build unless a later packaging packet changes loader policy.

Compute cost:
- Startup-only function pointer resolution.
- Zero per-frame cost after tables are loaded.
- No loader probing inside frame submission.

Verification:
- Unit tests cover active dynamic rendering name selection.
- Unit tests cover missing debug utils behavior for required and non-required validation modes.
- Smoke tests prove loaded swapchain and dynamic-rendering function names match selected feature path.
...
- File `src/render/vulkan/VulkanFunctions.cpp` has an implementation packet that follows this plan.
