...
Exact path: `tests/unit/render_diagnostics_tests.cpp`

Purpose: Prove renderer receipts are deterministic key-value text with stable reason fields.

## Build Position

Packet order: 1 - Backend-neutral renderer boundary
Owner module: `diagnostics tests`
File kind: `unit test`
...
This file owns:
- Prove renderer receipts are deterministic key-value text with stable reason fields.
...
receipt_version=1
repo=iggy3d
file_plan=tests/unit/render_diagnostics_tests.cpp
packet_order=1
...
fallback_area=diagnostics_tests
fallback_reason=
strict_vulkan=true|false
result=pass|fail|skip
reason_code=
...
- File `tests/unit/render_diagnostics_tests.cpp` has an implementation packet that follows this plan.
...
Test role: prove receipt formatting is deterministic and parser-friendly before Vulkan-specific receipts exist.

Required test groups:
```text
receipt_formats_one_key_value_per_line
receipt_preserves_field_order
receipt_normalizes_multiline_values
receipt_allows_equals_inside_values
receipt_required_common_fields_are_detected
render_outcome_names_are_stable
reason_codes_are_lowercase_snake_case
```

Required fixture receipt:
```text
receipt_version=1
repo=iggy3d
backend=null
...
- no Vulkan or SDL includes;
- no file IO;
- no timestamps in expected receipt strings;
- no unordered map output in golden receipt comparisons.

CTest labels:
```text
iggy3d;unit;render;diagnostics
```

Packet 1 completion proof:
```sh
ctest --test-dir build --output-on-failure -R 'render_diagnostics'
```
