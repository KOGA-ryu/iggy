...
Exact path: `src/render/vulkan/VulkanFeatureSupport.hpp`

Purpose: Declare feature, extension, format, queue, and unsupported-device gate evaluation.
...
- `docs/vulkan/vulkan_feature_query_chain.md`

## Ownership

This file owns:
- Declare feature, extension, format, queue, and unsupported-device gate evaluation.
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/VulkanFeatureSupport.hpp
packet_order=4
...
struct VulkanFeatureBaselineRequest {
    ...
    bool allowVulkan12DynamicRenderingExtensionPath;
    bool allowSoftwareDevice;
    bool requirePresentSupport;
    bool requireDepthFormat;
};
...
VulkanDeviceGateResult evaluateVulkanDeviceGates(
    VkPhysicalDevice physicalDevice,
    const VulkanFeatureBaselineRequest& request,
    const VulkanQueueFamilySelection& queues);

VulkanDeviceScore scoreVulkanDevice(
    const VulkanDeviceIdentity& identity,
    const VulkanQueueFamilySelection& queues,
...
Include policy:
- Vulkan-private header; Vulkan types are allowed.
- Does not include runtime, content, projection, save, SDL, or app platform headers.
- Device-scoring values exposed outside Vulkan only through diagnostics text.

Ownership:
- Owns feature baseline request shape, hard-gate evaluation result shape, and deterministic scoring result shape.
- Does not own actual device enumeration, logical device creation, or runtime policy.

Baseline contract:
- Dynamic rendering is required through core Vulkan 1.3 or the accepted Vulkan 1.2 extension path.
- Timeline semaphores are queried and kept disabled for first frame-loop packets.
- `VK_KHR_swapchain` is required for visual surface-backed backend startup.
- Portability subset is enabled when reported and required by the platform path.
- Software devices are rejected unless config explicitly allows software Vulkan lanes.

Verification:
- Unsupported-device unit tests exercise fake gate inputs without creating Vulkan objects.
- Feature baseline smoke prints the selected dynamic rendering source, timeline support, and portability fields.
...
- File `src/render/vulkan/VulkanFeatureSupport.hpp` has an implementation packet that follows this plan.
