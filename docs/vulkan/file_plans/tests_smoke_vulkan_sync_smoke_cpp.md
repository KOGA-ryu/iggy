...
Exact path: `tests/smoke/vulkan_sync_smoke.cpp`

Purpose: Prove frames-in-flight binary WSI synchronization, fence reset safety, and sync validation receipt.

## Build Position

Packet order: 5 - Swapchain, render loop, command recording, sync
Owner module: `sync smoke`
...
This file owns:
- Prove frames-in-flight binary WSI synchronization, fence reset safety, and sync validation receipt.
...
receipt_version=1
repo=iggy3d
file_plan=tests/smoke/vulkan_sync_smoke.cpp
packet_order=5
...
- File `tests/smoke/vulkan_sync_smoke.cpp` has an implementation packet that follows this plan.
