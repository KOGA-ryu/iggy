...
Exact path: `tests/smoke/vulkan_screenshot_smoke.cpp`

Purpose: Prove Vulkan image readback writes PNG/raw RGBA/meta artifacts with synchronization-clean capture.

## Build Position

Packet order: 7 - First visible room proof and packaging smoke
Owner module: `screenshot smoke`
...
This file owns:
- Prove Vulkan image readback writes PNG/raw RGBA/meta artifacts with synchronization-clean capture.
...
- `vulkan_screenshot_smoke executable`
...
receipt_version=1
repo=iggy3d
file_plan=tests/smoke/vulkan_screenshot_smoke.cpp
packet_order=7
...
fallback_area=screenshot_smoke
fallback_reason=
strict_vulkan=true|false
result=pass|fail|skip
reason_code=
...
Smoke purpose:
- Prove Vulkan readback can capture the first-room frame into deterministic screenshot artifacts.
- This smoke uses Vulkan copy/readback, not OS window screenshot automation.
- Screenshot artifacts are diagnostics only and never runtime truth.

Capture path:
1. Render a first-room frame.
2. Use a transfer-source swapchain image when swapchain usage supports it.
3. Use an offscreen capture target when swapchain transfer-source usage is unavailable.
4. Copy source image to a host-visible readback buffer.
5. Wait for copy completion through renderer sync policy.
6. Normalize row pitch and channel order to RGBA.
7. Write `.rgba`, `.png`, `.meta.kv`, and receipt files.
...
screenshot_raw_path=<path-or-empty>
screenshot_meta_path=<path-or-empty>
capture_source=<swapchain_transfer_src|offscreen_capture_target|unavailable>
...
Skip/fail policy:
- Non-strict lane may skip before capture when Vulkan/display prerequisites are absent.
- Strict screenshot lane fails when capture is required and unavailable, write fails, image readback fails, visible room is missing, or screenshot is entirely clear color.
- A failed screenshot capture must not change runtime hash or replay result.

Verification:
- Artifact paths in receipt must exist after pass.
- Image dimensions must match requested fixed smoke extent.
- Runtime/content/projection/save files must not read screenshot pixels.
...
- File `tests/smoke/vulkan_screenshot_smoke.cpp` has an implementation packet that follows this plan.
