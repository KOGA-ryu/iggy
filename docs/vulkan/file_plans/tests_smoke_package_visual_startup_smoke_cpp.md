...
Exact path: `tests/smoke/package_visual_startup_smoke.cpp`

Purpose: Prove installed or build-tree visual package resolves runtime dependencies, shader root, diagnostics dir, and exits cleanly.
...
This file owns:
- Prove installed or build-tree visual package resolves runtime dependencies, shader root, diagnostics dir, and exits cleanly.
...
- `package_visual_startup_smoke executable`
...
receipt_version=1
repo=iggy3d
file_plan=tests/smoke/package_visual_startup_smoke.cpp
packet_order=7
...
Smoke purpose:
- Prove build-tree and installed visual packages can locate resources, shaders, diagnostics dir, SDL/window support, and Vulkan backend startup policy.
- This smoke proves startup and one visual frame path when strict visual mode is enabled.

Modes:
```text
package_mode=build_tree_visual
package_mode=installed_visual
renderer_request=vulkan
renderer_request=null
```

Required behavior:
1. Resolve executable path through `ExecutablePath`.
2. Resolve package roots through `PackageRuntimeLookup`.
3. Resolve shader root and diagnostics dir.
4. Start visual demo with bounded frame count.
5. Use null renderer when graphics proof is not requested.
6. Use Vulkan renderer when strict visual package proof is requested.
7. Print package startup receipt.

Receipt fields:
```text
smoke=package_visual_startup
package_mode=<build_tree_visual|installed_visual>
executable_dir=<absolute-path>
resource_root=<absolute-path-or-empty>
resource_root_source=<source-name>
shader_root=<absolute-path-or-empty>
shader_root_source=<source-name>
diagnostics_dir=<absolute-path-or-empty>
renderer_request=<null|vulkan>
...
Skip/fail policy:
- Non-strict visual package lane may skip Vulkan startup when host graphics support is unavailable.
- Strict visual package lane fails on missing required resource root, shader root, SDL runtime, Vulkan loader, supported device, or malformed receipt.
- Null renderer package startup must not require Vulkan.

Verification:
- Installed package lookup must not depend on current working directory.
- Runtime hash/replay behavior remains unchanged by renderer startup.
...
- File `tests/smoke/package_visual_startup_smoke.cpp` has an implementation packet that follows this plan.
