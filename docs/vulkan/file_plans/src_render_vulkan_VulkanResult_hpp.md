...
Exact path: `src/render/vulkan/VulkanResult.hpp`

Purpose: Declare conversion from VkResult plus call context into backend-neutral render outcomes and reason codes.
...
This file owns:
- Declare conversion from VkResult plus call context into backend-neutral render outcomes and reason codes.
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/VulkanResult.hpp
packet_order=4
...
Include policy:
- This header may include Vulkan headers because it is Vulkan-private.
- It may include backend-neutral render result vocabulary.
- It must not include runtime, content, projection, save, SDL, or app platform headers.

Ownership:
- Owns raw `VkResult` to renderer outcome conversion declarations.
- Does not own smoke skip/fail exit mapping.
- Does not own recovery policy beyond flags needed by the backend and test boundary.

Semantics:
- `VK_SUCCESS` maps to `RenderOutcome::Pass` with `reason_code=vk_success`.
- Setup-time missing support maps to `RenderOutcome::Unsupported`; app/test wrappers translate that to skip or fail.
- `VK_ERROR_DEVICE_LOST` always maps to `device_lost`.
- Out-of-date and suboptimal swapchain results are not fatal by themselves and are contextual.
- Unknown raw values map to `vulkan_result_unmapped` and fail strict result-mapping tests.

Verification:
- `render_result_mapping_tests` cover every raw result name used by first-room Vulkan packets.
- `render_reason_code_tests` prove every mapped reason is stable and documented.
...
- File `src/render/vulkan/VulkanResult.hpp` has an implementation packet that follows this plan.
