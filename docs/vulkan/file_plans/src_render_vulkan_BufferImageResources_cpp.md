...
Exact path: `src/render/vulkan/BufferImageResources.cpp`

Purpose: Implement first-room vertex/index/depth resources, staging uploads, screenshot readback resources, and memory diagnostics.

## Build Position

Packet order: 6 - Shaders, pipeline, resources, descriptors
Owner module: `buffer image resources`
File kind: `source`
...
- `docs/vulkan/gpu_resource_upload.md`
- `docs/vulkan/vulkan_screenshot_and_frame_hash.md`

## Ownership

This file owns:
- Implement first-room vertex/index/depth resources, staging uploads, screenshot readback resources, and memory diagnostics.
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/BufferImageResources.cpp
packet_order=6
...
fallback_area=buffer_image_resources
fallback_reason=
...
Create behavior:
- Use `VulkanMemoryAllocator` for buffer and image allocations.
- Name every allocation before or immediately after creation.
- Upload first-room geometry through a host-visible staging buffer.
- Copy staging buffer into device-local vertex/index buffers.
- Create depth image with selected depth format and current swapchain extent.
- Create depth image view with depth aspect.
- Print allocation names and budget diagnostics.

First-room allocation names:
```text
buffer.first_room.vertices
buffer.first_room.indices
buffer.first_room.upload_staging
image.first_room.depth
```

Texture/material gate:
- Do not add texture image creation until descriptor reflection and texture format policy are active.
- Do not add material GPU records until descriptor layout compatibility is tested.
- No texture/material work may proceed without allocation naming and memory budget diagnostics.
...
depth_format_not_supported
depth_resource_create_failed
```

Compute cost:
- Startup allocates and uploads first-room geometry once.
- Resize recreates depth resources only when extent changes.
- Per-frame allocation count must stay zero.
- Staging resources are destroyed or recycled only after upload completion is safe.

Verification:
- Memory smoke asserts resource counts, allocation names, and per-frame allocation count.
- First-room smoke later asserts geometry resources are used by command recording.
...
- File `src/render/vulkan/BufferImageResources.cpp` has an implementation packet that follows this plan.
