...
Exact path: `tests/smoke/vulkan_feature_baseline_smoke.cpp`

Purpose: Prove selected device satisfies modern dynamic-rendering feature baseline or reports a precise unsupported result.

## Build Position

Packet order: 4 - Vulkan bootstrap, device, validation, diagnostics
Owner module: `feature smoke`
...
This file owns:
- Prove selected device satisfies modern dynamic-rendering feature baseline or reports a precise unsupported result.
...
- `vulkan_feature_baseline_smoke executable`
...
receipt_version=1
repo=iggy3d
file_plan=tests/smoke/vulkan_feature_baseline_smoke.cpp
packet_order=4
...
Smoke purpose:
- Prove the selected physical device satisfies the Vulkan feature baseline needed before swapchain and first-room packets.
- This smoke is device/feature proof only; it does not submit rendering commands.

Feature checks:
- API target reports Vulkan 1.3 support or the accepted Vulkan 1.2 dynamic-rendering extension path.
- Dynamic rendering support is present and selected.
- Timeline semaphore support is queried and remains disabled.
- `VK_KHR_swapchain` is present for visual surface-backed startup.
- Required first-room limits are present, including minimum push constant bytes.
- Required depth format support is present when first-room depth is enabled.
- Apple portability fields are reported on macOS/MoltenVK.

Receipt fields:
```text
smoke=vulkan_feature_baseline
api_baseline=Vulkan_1_3_preferred
api_compatibility_floor=Vulkan_1_2_plus_required_extensions
...
swapchain_extension_supported=true|false
min_push_constant_bytes_required=64
min_push_constant_bytes_supported=<integer-or-none>
depth_format_supported=true|false|unavailable
...
Skip/fail policy:
- Non-strict feature smoke may skip before device creation when Vulkan setup is unavailable.
- Strict feature smoke fails on missing required baseline support.
- Dynamic rendering missing fails strict lanes; render-pass fallback requires a separate documented decision before use.

Verification:
- Smoke output must match `VulkanFeatureSupport` gate decisions.
- Smoke must prove no timeline semaphore feature is enabled in first Packet 4 path.
- Smoke must run on macOS, Linux, and Windows lanes before first-room renderer status is considered shippable.
...
- File `tests/smoke/vulkan_feature_baseline_smoke.cpp` has an implementation packet that follows this plan.
