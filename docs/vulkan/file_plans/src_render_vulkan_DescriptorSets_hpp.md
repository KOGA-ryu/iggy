...
Exact path: `src/render/vulkan/DescriptorSets.hpp`

Purpose: Declare descriptor layouts, pools, material/texture bindings, and first descriptor growth gates.
...
This file owns:
- Declare descriptor layouts, pools, material/texture bindings, and first descriptor growth gates.
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/DescriptorSets.hpp
packet_order=6
...
enum class DescriptorLayoutFamily {
    None,
...
First-room policy:
- First-room baseline uses no descriptor sets.
- `descriptor_set_layout_count=0`.
- `DescriptorLayoutFamily::None` is valid and expected for first-room pipeline smoke.

Material descriptor policy:
```text
...
Ownership:
- Owns descriptor set layouts, descriptor pools, descriptor allocations, and material descriptor update records.
- Does not own runtime material truth, content validation, texture upload, sampler creation, pipeline creation, or shader reflection tooling.

Gate:
- Material descriptor implementation requires generated SPIR-V reflection or verified generated manifest.
- C++ hardcoded descriptor layout is allowed only when reflection tests verify it against generated SPIR-V.

Verification:
- Descriptor smoke proves first material layout only after reflection gate passes.
- Pipeline smoke proves first-room path has zero descriptor sets.
...
- File `src/render/vulkan/DescriptorSets.hpp` has an implementation packet that follows this plan.
