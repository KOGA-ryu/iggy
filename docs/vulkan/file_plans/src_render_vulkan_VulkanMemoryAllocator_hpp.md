...
Exact path: `src/render/vulkan/VulkanMemoryAllocator.hpp`

Purpose: Declare VMA allocator wrapper, allocation naming, budget snapshot, and teardown contract for resource growth.
...
This file owns:
- Declare VMA allocator wrapper, allocation naming, budget snapshot, and teardown contract for resource growth.
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/VulkanMemoryAllocator.hpp
packet_order=6
...
struct VulkanMemoryBudgetReceipt {
    bool budgetAvailable;
    std::uint64_t heapBudgetBytes;
    std::uint64_t heapUsageBytes;
...
public:
    ...
    VulkanMemoryBudgetReceipt budgetReceipt() const;
    RenderReceipt setAllocationName(VmaAllocation allocation, std::string_view name);
};
...
Include policy:
- Vulkan and VMA headers are allowed only under `src/render/vulkan/**`.
- Runtime, content, projection, save, SDL, and app platform headers are forbidden.

Ownership:
- Owns `VmaAllocator` lifetime, VMA budget/stat diagnostics, allocation naming helper, and allocator-level result mapping.
- Does not own buffer/image semantic records, texture formats, runtime assets, content package truth, or descriptor binding.

Policy:
- VMA is required before resource growth beyond bootstrap.
- Every allocation must have a stable debug name.
- Budget diagnostics are printed at allocator creation, after first resources, and during memory smoke.
- Defragmentation is deferred and disabled in Packet 6.

Verification:
- Memory budget unit tests prove allocation names and budget receipt schema.
- Vulkan memory smoke proves allocator creation/destruction and budget receipt fields.
...
- File `src/render/vulkan/VulkanMemoryAllocator.hpp` has an implementation packet that follows this plan.
