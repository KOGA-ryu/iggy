...
Exact path: `src/render/vulkan/VulkanBackend.hpp`

Purpose: Declare the Vulkan RenderBackend implementation facade hidden behind the backend-neutral renderer API.
...
This file owns:
- Declare the Vulkan RenderBackend implementation facade hidden behind the backend-neutral renderer API.
...
receipt_version=1
repo=iggy3d
file_plan=src/render/vulkan/VulkanBackend.hpp
packet_order=4
...
public:
    ...
    RenderReceipt initialize(const FrameInput& firstFrame) override;
    RenderReceipt submitFrame(const FrameInput& frameInput) override;
    RenderReceipt resize(std::uint32_t drawableWidth, std::uint32_t drawableHeight) override;
    RenderReceipt waitIdle() override;
    RenderReceipt shutdown() override;
    RenderBackendName backendName() const override;
};
...
Public boundary:
- The class implements `RenderBackend` and is constructed only by renderer-owned factory code.
- Public methods use backend-neutral `FrameInput`, `RendererConfig`, `RenderReceipt`, and diagnostics types.
- Raw Vulkan handles, `VkResult`, `PFN_vk*`, validation callback data, and platform WSI types remain private to `src/render/vulkan/**`.

Ownership:
- Owns module composition for the Vulkan backend.
- Delegates Vulkan handles to `InstanceDeviceSurface`, validation/debug messenger state to `DebugValidation`, and function tables to `VulkanFunctions`.
- Does not own runtime truth, projection creation, save/replay data, package validation, SDL windows, or app event polling.

Packet 4 scope:
- `initialize()` creates enough Vulkan state for device diagnostics smoke: instance, surface, physical device selection, logical device, queue handles, validation state, and function loading receipts.
- `submitFrame()` may return a diagnosed not-rendering receipt until swapchain and command packets land.
- `resize()` records drawable extent intent but performs no swapchain work in Packet 4.
- `shutdown()` tears down Packet 4 Vulkan state in reverse ownership order.
...
Compute cost:
- Initialization may enumerate layers, extensions, physical devices, queue families, and device features once.
- Per-frame Packet 4 cost is zero GPU submission.
- Shutdown cost is bounded Vulkan object teardown and diagnostics flush.

Verification:
- `vulkan_device_smoke` proves `initialize()` and `shutdown()` work without runtime mutation.
- `vulkan_validation_smoke` proves validation config reaches the backend.
- Boundary tests prove this header does not expose Vulkan types outside the private backend surface.
...
- File `src/render/vulkan/VulkanBackend.hpp` has an implementation packet that follows this plan.
