# E256: Product Receipt Field Rows G6 - Startup Probe
...
This is the sixth receipt-field row slice after E251-E255. It should remain a
...
The receipt row-table pattern has now been applied successfully to five direct
receipt sections:
...
E250 classified `StartupProbeFields.cpp` as a direct table candidate:

- 84 lines
- 38 `appendReceiptField(...)` rows
...
- values are direct bool/string/count/microsecond fields plus the existing
  `frontendDevToolsCategoryName(...)` stringifier
...
`StartupProbeFields.cpp`.

Preserve the existing public API:

```cpp
void appendProductStartupProbeFields(RenderReceipt& receipt,
...
Preserve every current row from `appendProductStartupProbeFields(...)`:
...
The table must cover all 38 current rows. Do not leave direct procedural rows
...
```sh
rg -n "StartupProbeReceiptContext|StartupProbeReceiptFieldRow|kStartupProbeReceiptFields|appendProductStartupProbeFields|appendReceiptField\\(" \
  /Users/kogaryu/iggy3d/src/app/iggy3d/receipt/StartupProbeFields.cpp
```
...
- `appendProductStartupProbeFields(...)` remains the only public appender
...
```sh
rg -n "ProductAppReceiptContext|ReceiptFieldRow|StartupProbeReceiptFieldRow" \
...
- `StartupProbeReceiptFieldRow` appears only in `StartupProbeFields.cpp`

Run:

```sh
rg -c "appendReceiptField\\(" /Users/kogaryu/iggy3d/src/app/iggy3d/receipt/StartupProbeFields.cpp
```
...
- direct bool/string/count/microsecond formatting, overload selection, or
  stringifier output changes
- compile fallout expands beyond include repairs in `StartupProbeFields.cpp`
