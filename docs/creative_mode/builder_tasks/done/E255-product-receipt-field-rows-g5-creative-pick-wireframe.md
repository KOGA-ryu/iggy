# E255: Product Receipt Field Rows G5 - Creative Pick Wireframe
...
This is the fifth receipt-field row slice after E251-E254. It should remain a
...
The receipt row-table pattern has now been applied successfully to four direct
receipt sections:
...
E250 classified `CreativePickWireframeFields.cpp` as a direct table candidate:

- 136 lines
- 52 `appendReceiptField(...)` rows
- one appender function
- no loops
- no conditionals
- values come from:
  - `ProductAppWindowState::creativeAuthoring`
  - `ProductVulkanGameplayReadiness`
- values are direct bool/string/count fields plus existing `std::to_string(...)`
  formatting for pick coordinates
...
`CreativePickWireframeFields.cpp`.

Preserve the existing public API:

```cpp
void appendProductCreativePickWireframeFields(
        ...
        {"creative_viewport_pick_requested",
         [](RenderReceipt& receipt,
           ...
           appendReceiptField(receipt, key,
                              context.authoring.creativeViewportPickRequested);
         }},
...
`appendProductCreativePickWireframeFields(...)`:

- same key literal
- same order
- same value expression
- same `std::to_string(...)` formatting
...
The table must cover all 52 current rows. Do not leave direct procedural rows
...
```sh
rg -n "CreativePickWireframeReceiptContext|CreativePickWireframeReceiptFieldRow|kCreativePickWireframeReceiptFields|appendProductCreativePickWireframeFields|appendReceiptField\\(" \
  /Users/kogaryu/iggy3d/src/app/iggy3d/receipt/CreativePickWireframeFields.cpp
```
...
- `appendProductCreativePickWireframeFields(...)` remains the only public
  appender
...
```sh
rg -n "ProductAppReceiptContext|ReceiptFieldRow|CreativePickWireframeReceiptFieldRow" \
...
- `CreativePickWireframeReceiptFieldRow` appears only in
  `CreativePickWireframeFields.cpp`

Run:

```sh
rg -c "appendReceiptField\\(" /Users/kogaryu/iggy3d/src/app/iggy3d/receipt/CreativePickWireframeFields.cpp
```
...
- `std::to_string(...)`, direct bool/string/count formatting, overload
  selection, or stringifier output changes
