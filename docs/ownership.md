...
- local multiplayer merge: `LocalMultiplayerSession`, O(submitted command count
  times current command-log record count) in the complete build because duplicate
  `commandId` checks are deterministic linear scans.
