...
| AUT-002 | Atomic document mutation and rollback | Integrated | Planned | `DocumentMutation` is canonical; 22 direct staged assignments compete with three `commitStagedMutation` call sites and same-document `installDocument` paths | Make same-document publication singular and pin one revision per atomic operation |
...
| AUT-008 | Close mutable Facade document escape | Integrated | In Progress | Baseline `bbd1c76f` had 17 production and 30 test callers; all are migrated to explicit mutation and hierarchy contracts, with the accessor deleted. The focused attachment executable remains blocked by the pre-existing app archive link closure | Review the bounded commit, then rerun the prescribed attachment build when the app archive exports its existing symbols |
