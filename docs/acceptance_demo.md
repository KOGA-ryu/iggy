...
- `gold_key` reach distance is measured to its entity transform position;
  bounds do not affect complete-build reach in this demo;
...
`Session` assigns stable nonzero command ids before admission; `CommandLog`
assigns deterministic sequence numbers during append. The acceptance test maps
stable labels to the `Session`-assigned ids after submission:
...
command labels above and are not appended to the saved source gameplay command
log.
...
- `SaveSessionSection.nextCommandId` equal to the next id after the ten gameplay
  command records;
- state hash metadata.
...
5. verify restored `nextCommandId` is greater than every restored command record
   `commandId`;
6. submit one normal post-load gameplay command through `Session::submitCommand`
   on a proof branch;
7. verify that command receives the restored `nextCommandId`, does not collide
   with any restored command id, and advances the cursor by one only after
   `CommandLog::append` returns `Ok`;
8. compare loaded hash to saved hash;
9. compare loaded summary to source summary.

The post-load cursor proof is diagnostic proof only. It must not add save, load,
reset, or post-load proof events to the ten gameplay command count for the
source demo session.
...
- inventory empty;
- objective `collect_gold_key=Active`;
- command log empty after `CommandLogResetPolicy::Clear`;
- `nextCommandId == 1`;
- pending execution queue empty;
- reset hash equals initial baseline hash.

Reset proof is a dedicated API/event phase. It is not submitted or logged as a
gameplay command record and must not change the ten-command source log counts.
...
- verify final hash;
- compare expected summary when `--expect-summary` is provided;
- the acceptance/demo invocation must pass `--expect-summary`, and summary
...
The acceptance test must call public library APIs directly and must also execute
the headless demo binary when that target is built. It must not require a
renderer.
...
## Locked First-Build Choices

These first-build choices are locked for code, tests, and expected summary:

- objective completion sets objective status and outcome `DemoComplete`, while
  lifecycle remains `Playing` until the scripted gameplay phase finishes; final
  summary reports lifecycle/outcome as completed after the final gameplay
  command and idle drain;
- command-log records, next sequence, and epoch affect `StateHash`;
- reset clears command history with `CommandLogResetPolicy::Clear`;
- exact final tick count;
- exact final state hash.

Only final tick count and final state hash remain fixture values to fill after
implementation produces the deterministic proof.
