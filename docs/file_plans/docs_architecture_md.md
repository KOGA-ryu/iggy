...
- `core` depends on the C++ standard library only in the complete demo build.
  Future core dependencies require an explicit build-doc contract update.
...
5. run deterministic AI proposal generation; the complete demo returns no AI
   proposals for all policy ids and mutates no world state;
...
- AI proposals are command proposals for post-demo admission work, not direct
  mutation; the complete demo emits no AI proposals;
...
- checks actor transform position against the target entity transform position or
  explicit command target point; complete-build reach does not use bounds;
