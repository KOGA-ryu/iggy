# `tests/unit/projection_tests.cpp`

Updated: 2026-06-20

Exact purpose: prove scene/debug projection behavior for the complete `iggy3d` runtime.

## Build Position

- priority rank: 108
- tier: Tier 6: Full Session Loop Diagnostics Projection
- module: `unit tests`
- file kind: `test`

## Ownership

This file owns:

- test scenarios
- assertions
- fixture setup helpers local to this test file
- regression coverage for documented semantics

It must not own:

- no includes, links, generated code, or schema adapters from old `/Users/kogaryu/iggy`;
- no renderer-owned gameplay truth;
- no raw input events persisted as gameplay commands;
- no hidden global mutable state;
- no nondeterministic time, random, filesystem, or container-order behavior inside runtime logic.

## Allowed Dependencies

- public iggy3d headers
- test framework chosen by `cmake/iggy3d_tests.cmake`
- fixtures under `/Users/kogaryu/iggy3d/fixtures`

## Data Contract

- projection creates scene items for active entities
- inactive pickup removed after interaction
- camera state projected read-only
- debug projection includes reach/target facts

## Semantics

- tests must be deterministic
- tests must not require renderer or old iggy code
- tests should assert exact rejection/status codes when behavior is part of runtime contract

## Detailed Design Contract

Test cases required:

- initial first-room projection emits one player, active `gold_key`, the
  interactable target, and `tactical_marker_alpha` in deterministic order;
- inactive `gold_key` is excluded by default after collection and included only
  when `includeInactive` is true;
- building scene projection does not mutate tick, hash, command log, metrics,
  events, inventory, objective, or entity active flags;
- debug projection includes `OutOfRange` marker for `cmd_interact_oob` and
  clock/camera markers after tactical slow-time camera command;
- projection rebuilt after save/load matches equivalent runtime truth but is
  not loaded from the save envelope;
- tests use library APIs only, no renderer/window/GPU/network/old iggy code.

Assertions should compare stable ids, counts, order, kind, active flags, and
source hash/tick. They should not assert localized text or memory addresses.

## Implementation Plan

1. include the test framework and only public `iggy3d` headers required for the scenario;
2. build test state through public APIs or fixture loaders;
3. assert the exact success, failure, rejection, and deterministic replay behavior named in this document;
4. keep the test independent of renderer, network services, wall-clock timing, and old `iggy` code.

## Compute Cost

- Test runtime should stay small; fixture-level tests may scan all demo entities and commands.
- Any future optimization must preserve deterministic ordering, command replay, and state hash behavior.

## Diagnostics And Errors

- assert expected failures through this file's declared machine-readable result contract;
- do not use logging text as the only machine-readable outcome;
- surface enough context for acceptance and replay failures to identify the failing command, entity, or file.

## Save Replay Multiplayer Notes

- if this file owns save truth, it must define exact fields included in `SaveEnvelope`;
- if this file owns derived data, it must be regenerable and excluded from save truth;
- if this file affects commands, replay must reproduce the same result and state hash.

## Tests And Verification

- paired unit or acceptance test listed in `PRIORITY.md` must cover this file where behavior is nontrivial;
- success and failure paths must be deterministic;
- no test may depend on renderer, wall-clock timing, network service, or old iggy code.

## Completion Criteria

- `tests/unit/projection_tests.cpp` exists in `/Users/kogaryu/iggy3d`;
- it builds without old `iggy` dependencies;
- its behavior is covered by the ranked test or acceptance file;
- it follows `docs/architecture.md` and `docs/ownership.md` without redefining ownership.
