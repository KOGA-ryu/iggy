...
`Session` command construction assigns each stable nonzero
`CommandRecord::commandId` before lifecycle prefiltering and admission.
`CommandLog::append` assigns `CommandRecord::sequence` when unset. The
acceptance test must be able to identify commands by stable labels:
