...
bool saveExitWithPlacedFloor(const iggy3d::smoke::ReceiptFields& fields) {
...
bool continueRestoresPlacedFloor(const iggy3d::smoke::ReceiptFields& fields) {
  return iggy3d::smoke::productReceipt(fields) &&
         iggy3d::smoke::automationApplied(fields) &&
...
int main() {
  ...
  const std::filesystem::path saveRoot =
      iggy3d::smoke::cleanSaveRoot("editor_floor_save_continue");
  ...
  const bool saved =
      iggy3d::smoke::runProductCase(
          binary,
          "editor_floor_save_continue_save_exit",
          ...
          fields,
          exitCode) &&
      exitCode == 0 && saveExitWithPlacedFloor(fields) &&
      ...
      fileContains(saveFile, "authoredRoom.wall.count=61\n");
  ...
  const bool rebootStarter =
      ...
      iggy3d::smoke::runProductReceiptCase(
          binary,
          "editor_floor_save_continue_reboot_starter",
          iggy3d::smoke::saveRootArg(saveRoot),
          fields,
          exitCode) &&
  ...
  const bool continued =
      ...
      iggy3d::smoke::runProductCase(
          binary,
          "editor_floor_save_continue_continue",
          "frontend.select=continue\nfrontend.execute=true\n",
          iggy3d::smoke::saveRootArg(saveRoot),
          fields,
          exitCode) &&
      exitCode == 0 && continueRestoresPlacedFloor(fields);

  const bool ok =
      expect(saved, "editor input floor save and exit") &&
      expect(rebootStarter, "fresh starter sees floor-edited save") &&
      expect(continued, "continue restores floor-edited room");

  std::cout << "editor_floor_save_continue_save_exit="
            << (saved ? "true" : "false") << '\n';
  std::cout << "editor_floor_save_continue_reboot_starter="
            << (rebootStarter ? "true" : "false") << '\n';
  std::cout << "editor_floor_save_continue_restored="
