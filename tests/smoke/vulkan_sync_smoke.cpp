...
iggy3d::RenderReceipt baseReceipt(std::string_view result, std::string_view reason) {
  ...
  iggy3d::appendReceiptField(receipt, "smoke", "vulkan_sync");
  iggy3d::appendReceiptField(receipt, "packet_order", "5");
  iggy3d::appendReceiptField(receipt, "backend", "vulkan");
  iggy3d::appendReceiptField(receipt, "sync_policy", "binary_wsi");
  iggy3d::appendReceiptField(receipt, "frame_slots", static_cast<std::uint64_t>(2));
...
iggy3d::FrameInput validFrame(const iggy3d::SceneProjectionResult& scene,
  ...
  frame.clock = {frameIndex, frameIndex, 0.0F, 1.0F / 60.0F};
...
int main() {
  ...
  create.title = "iggy3d sync smoke";
  create.width = 640U;
  create.height = 360U;
  create.vulkan = true;
  ...
  const iggy3d::RenderSubmitResult first =
      backend.submitFrame(validFrame(scene, extent.width, extent.height, 1U));
  const iggy3d::RenderSubmitResult second =
      backend.submitFrame(validFrame(scene, extent.width, extent.height, 2U));
  iggy3d::RenderReceipt receipt = second.receipt;
  iggy3d::appendReceiptField(receipt, "smoke", "vulkan_sync");
  iggy3d::appendReceiptField(receipt, "first_frame_ok",
                             first.outcome == iggy3d::RenderOutcome::Ok);
  iggy3d::appendReceiptField(receipt, "second_frame_ok",
                             second.outcome == iggy3d::RenderOutcome::Ok);
  backend.shutdown();
  const bool ok = first.outcome == iggy3d::RenderOutcome::Ok &&
                  second.outcome == iggy3d::RenderOutcome::Ok;
  return printReceipt(ok ? receipt : baseReceipt("fail", "sync_validation_failed"), ok ? 0 : 1);
#endif
}
