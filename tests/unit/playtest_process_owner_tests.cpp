...
bool waitForHeartbeatState(app::PlaytestProcessOwner& owner,
                           std::string_view state,
                           std::uint64_t afterReceiptMs,
                           double timeoutSeconds,
                           std::uint64_t& tick,
                           std::uint64_t& receiptMs) {
  const Clock::time_point start = Clock::now();
  while (secondsSince(start) < timeoutSeconds) {
    static_cast<void>(owner.poll());
    const app::PlaytestMonitorState& monitor = owner.monitor();
    if (monitor.lastHeartbeatAtMs != 0U &&
        monitor.lastHeartbeatAtMs != afterReceiptMs &&
        monitor.lastHeartbeatState == state) {
      tick = monitor.lastHeartbeatTick;
      receiptMs = monitor.lastHeartbeatAtMs;
      return true;
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(50));
  }
...
bool commandChannelPauseResumeAcks(const std::string& i3dpPath,
  ...
  std::uint64_t pausedTickA = 0U;
  std::uint64_t pausedReceiptA = 0U;
  const std::uint64_t beforePausedReceipt =
      owner.monitor().lastHeartbeatAtMs;
  if (!expect(waitForHeartbeatState(owner, "suspended", beforePausedReceipt,
                                    5.0, pausedTickA, pausedReceiptA),
              "first paused heartbeat arrives")) {
    return false;
  }
  std::uint64_t pausedTickB = 0U;
  std::uint64_t pausedReceiptB = 0U;
  if (!expect(waitForHeartbeatState(owner, "suspended", pausedReceiptA, 5.0,
                                    pausedTickB, pausedReceiptB),
              "second paused heartbeat arrives")) {
    return false;
  }
  std::cout << "  pause: state=suspended tick " << pausedTickA
            << " -> " << pausedTickB << "\n";
  if (!expect(pausedReceiptB != pausedReceiptA,
              "paused heartbeat receipts are distinct") ||
  ...
  std::uint64_t resumedTick = 0U;
  std::uint64_t resumedReceipt = 0U;
  if (!expect(waitForHeartbeatState(owner, "running", pausedReceiptB, 5.0,
                                    resumedTick, resumedReceipt),
              "resumed heartbeat arrives")) {
    return false;
  }
  std::cout << "  resume: state=running tick " << resumedTick << "\n";
  if (!expect(resumedReceipt != pausedReceiptB,
              "resumed heartbeat receipt is fresh") ||
