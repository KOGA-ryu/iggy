...
bool rotatedBoundsProjectionUsesResolvedWorldAabb() {
  cr::CreativeObject wall;
  wall.id = 91U;
  wall.kind = cr::CreativeObjectKind::Wall;
  wall.transform.position = {3.0, 1.0, 3.0};
  wall.transform.rotationEulerRadians.y = std::numbers::pi * 0.5;
  wall.bounds = {{1.0, 0.0, 2.875}, {5.0, 2.0, 3.125}};
  const cr::CreativeTransformedBounds resolved =
      cr::resolveCreativeObjectBounds(wall);
  const cr::CreativeSpatialProjectionRequest request = makeDeepRequest();
  const cr::CreativeSpatialProjectionSummary summary =
      cr::projectObjectToGridSummary(wall, request);
  const cr::CreativeGridBounds3 expected = cr::worldBoundsToGridBounds(
      resolved.worldBounds, request.cellSize);
  return expect(resolved.valid,
                "rotated projection resolves transformed geometry") &&
         expect(summary.status == cr::CreativeSpatialProjectionStatus::Projected,
                "rotated projection remains projectable") &&
         expect(sameBounds(summary.projectedBounds, expected),
                "spatial projection consumes transformed world AABB");
}
...
int main() {
  const bool ok = rowMajorIndexRoundTripIsStable() &&
                  ...
                  rotatedBoundsProjectionUsesResolvedWorldAabb();
