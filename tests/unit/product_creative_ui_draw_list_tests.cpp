...
const iggy3d::CreativeUiPrimitive* findPrimitive(
    const iggy3d::CreativeUiDrawList& list,
    std::string_view semanticId) {
  for (const iggy3d::CreativeUiPrimitive& primitive : list.primitives) {
...
const iggy3d::CreativeUiHitRegion* findHitRegion(
    const iggy3d::CreativeUiDrawList& list,
    std::string_view semanticId) {
  for (const iggy3d::CreativeUiHitRegion& hit : list.hitRegions) {
...
std::size_t primitiveIndex(const iggy3d::CreativeUiDrawList& list,
                           std::string_view semanticId) {
...
bool rectEquals(const iggy3d::CreativeUiRect& lhs,
                const iggy3d::CreativeUiRect& rhs) {
  return lhs.x == rhs.x && lhs.y == rhs.y && lhs.width == rhs.width &&
         lhs.height == rhs.height;
}

bool rectsOverlap(const iggy3d::CreativeUiRect& lhs,
                  const iggy3d::CreativeUiRect& rhs) {
  return lhs.x < rhs.x + rhs.width && lhs.x + lhs.width > rhs.x &&
         lhs.y < rhs.y + rhs.height && lhs.y + lhs.height > rhs.y;
}

bool rectInsideVirtualFrame(const iggy3d::CreativeUiRect& rect,
                            std::uint32_t width,
                            std::uint32_t height) {
...
bool textFitsPrimitiveRect(const iggy3d::CreativeUiPrimitive& primitive) {
  constexpr float kGlyphAdvance = 12.0F;
  return static_cast<float>(primitive.text.size()) * kGlyphAdvance <=
         primitive.rect.width;
}

std::uint64_t countKind(const iggy3d::CreativeUiDrawList& list,
                        iggy3d::CreativeUiPrimitiveKind kind) {
...
bool rowHitMatchesTextPrimitive(const iggy3d::CreativeUiDrawList& list,
                                std::string_view semanticId) {
  const iggy3d::CreativeUiPrimitive* primitive =
      findPrimitive(list, semanticId);
  const iggy3d::CreativeUiHitRegion* hit = findHitRegion(list, semanticId);
  return expect(primitive != nullptr, "matching primitive exists") &&
         expect(hit != nullptr, "matching hit exists") &&
         expect(primitive->kind == iggy3d::CreativeUiPrimitiveKind::Text,
                "matching primitive text") &&
         expect(hit->semanticId == primitive->semanticId,
                "matching semantic") &&
         expect(rectEquals(hit->rect, primitive->rect), "matching rect") &&
         expect(hit->kind == iggy3d::CreativeUiHitKind::Row,
                "matching hit row kind") &&
...
bool nullModelFailsClosed() {
  ...
  request.theme = iggy3d::CreativeUiThemeId::Journal;
  const iggy3d::CreativeUiDrawList list =
  ...
  return expect(!list.ready, "null not ready") &&
         ...
         expect(list.theme == iggy3d::CreativeUiThemeId::Journal,
                "null theme") &&
...
bool defaultModelProducesReadyDrawList() {
  ...
  return expect(list.ready, "default ready") &&
         ...
         expect(list.theme == iggy3d::CreativeUiThemeId::System,
                "default theme");
}
...
bool primitivesAreNonInteractiveDocumentSemantics() {
  ...
  for (const iggy3d::CreativeUiHitRegion& hit : list.hitRegions) {
    ok = expect(hasPrefix(hit.semanticId, "creative."),
         ...
         ok;
    ok = expect(hit.kind == iggy3d::CreativeUiHitKind::Row, "hit row kind") &&
         ok;
  }
...
bool defaultModelUsesSeparatedPanelZones() {
  ...
  const iggy3d::CreativeUiPrimitive* tools =
      findPrimitive(list, "creative.panel.tools");
  const iggy3d::CreativeUiPrimitive* create =
...
bool populatedModelPreservesCreativeOrderAndText() {
  ...
  return expect(selected != nullptr &&
                    selected->text == "Selected: count=1 primary=42 visible=true",
                "selected row text") &&
...
bool populatedModelTextStaysInsidePrimitiveRects() {
  ...
  for (const iggy3d::CreativeUiPrimitive& primitive : list.primitives) {
    ok = expect(rectInsideVirtualFrame(primitive.rect, 1280, 720),
         ...
         ok;
    if (primitive.kind == iggy3d::CreativeUiPrimitiveKind::Text) {
...
bool selectedTargetVisibilityTextHandlesHiddenAndUnknown() {
  ...
  const iggy3d::CreativeUiDrawList hiddenList =
      iggy3d::buildProductCreativeUiDrawList(hiddenRequest);
  const iggy3d::CreativeUiPrimitive* hidden =
  ...
  const iggy3d::CreativeUiDrawList missingList =
      iggy3d::buildProductCreativeUiDrawList(missingRequest);
  const iggy3d::CreativeUiPrimitive* missing =
  ...
  return expect(hidden != nullptr &&
         ...
         expect(hidden != nullptr &&
                    hidden->text == "Selected: count=1 primary=42 visible=false",
                "hidden selected text") &&
         expect(missing != nullptr &&
                    missing->text == "Selected: count=1 primary=77 visible=unknown",
...
bool countersMatchPrimitiveContents() {
  ...
  const std::uint64_t textCount =
      countKind(list, iggy3d::CreativeUiPrimitiveKind::Text);
  const std::uint64_t panelCount =
      countKind(list, iggy3d::CreativeUiPrimitiveKind::Panel);
...
bool disabledRowsEmitDisabledHitRegions() {
  ...
  const iggy3d::CreativeUiPrimitive* active =
      findPrimitive(list, "creative.row.tools.tool_select");
  const iggy3d::CreativeUiHitRegion* activeHit =
...
bool undoRowAvailabilityControlsHitRegion() {
  ...
  const iggy3d::CreativeUiDrawList disabledList =
      iggy3d::buildProductCreativeUiDrawList(disabledRequest);
  const iggy3d::CreativeUiPrimitive* disabledPrimitive =
      findPrimitive(disabledList, "creative.row.tools.undo");
  const iggy3d::CreativeUiHitRegion* disabledHit =
  ...
  const iggy3d::CreativeUiDrawList enabledList =
      iggy3d::buildProductCreativeUiDrawList(enabledRequest);
  const iggy3d::CreativeUiPrimitive* enabledPrimitive =
      findPrimitive(enabledList, "creative.row.tools.undo");
  const iggy3d::CreativeUiHitRegion* enabledHit =
...
bool repeatedBuildIsStable() {
  ...
  iggy3d::ProductCreativeUiDrawListRequest request;
  request.model = &model;
  const iggy3d::CreativeUiDrawList first =
      iggy3d::buildProductCreativeUiDrawList(request);
  const iggy3d::CreativeUiDrawList second =
      iggy3d::buildProductCreativeUiDrawList(request);
