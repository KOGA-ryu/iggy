...
void showMovementTuning(StarterHarness& harness) {
  harness.window.gameplay.gameplayMovement.tuningVisible = true;
  harness.window.gameplay.gameplayMovement.tuningStatus = "movement_tuning_visible";
  harness.window.gameplay.gameplayMovement.tuningReasonCode =
      harness.window.gameplay.gameplayMovement.tuningStatus;
}
...
bool childPanelActionsOpenExpectedSurfaces() {
  ...
  return expect(newWorldResult.handled && newWorldResult.accepted,
         ...
         expect(newWorld.window.creativeAuthoring.worldSetup.status == "world_setup_open",
                "new world records draft state") &&
         expect(!newWorld.window.gameplay.gameplayMovement.tuningVisible,
         ...
         expect(loadSave.window.saveSession.selectedProductSave.id == "save_unit",
                "load selected save") &&
         expect(!loadSave.window.gameplay.gameplayMovement.tuningVisible,
         ...
         expect(!settings.window.gameplay.gameplayMovement.tuningVisible,
         ...
         expect(!devTools.window.gameplay.gameplayMovement.tuningVisible,
...
bool deleteAndExitActionsAreExplicitRows() {
  ...
  return expect(deleteResult.handled && deleteResult.accepted,
         ...
         expect(!deleteSave.window.gameplay.gameplayMovement.tuningVisible,
         ...
         expect(!exit.window.gameplay.gameplayMovement.tuningVisible,
...
bool creativeNewWorldLaunchesThroughStarterActionAndKeepsContinueSeparate() {
  ...
  return expect(result.handled && result.accepted,
         ...
         expect(harness.window.inputDevice.interactionMode ==
                    iggy3d::ProductInteractionMode::Creative,
                "creative starter interaction mode") &&
         expect(harness.window.frontendShell.launchStatus ==
                    "product_creative_world_launched",
                "creative starter launch status") &&
         expect(!harness.window.gameplay.gameplayMovement.tuningVisible,
...
bool creativeOpenWorldLaunchesThroughStarterAction() {
  ...
  return expect(created.accepted, "creative open setup create accepted") &&
         ...
         expect(app.identity.saveId == created.saveId,
                "creative open active creative save id") &&
         expect(app.identity.savePath ==
                    created.path.generic_string(),
                "creative open active creative save path") &&
         expect(app.identity.worldId == created.worldId,
                "creative open active creative world id") &&
         expect(app.identity.documentId == created.documentId,
                "creative open active creative document id") &&
         expect(app.identity.objectCount == 0U,
...
bool creativeWorldMinimumLifecycleLoopsThroughStarterCreateSaveExitAndOpen() {
  ...
  const iggy3d::ProductInteractionMode launchInteractionMode =
      harness.window.inputDevice.interactionMode;
  const std::string launchStatus = harness.window.frontendShell.launchStatus;
  const std::string launchedSaveId = app.identity.saveId;
  const std::string launchedSavePath = app.identity.savePath;
  const std::string launchedWorldId = app.identity.worldId;
  ...
  const iggy3d::ProductInteractionMode saveExitInteractionMode =
      harness.window.inputDevice.interactionMode;
  const std::string saveExitLaunchStatus = harness.window.frontendShell.launchStatus;
  ...
  const bool creativeIdentityCleared =
      ...
      app.identity.savePath == "none" &&
      app.identity.worldId == "none" &&
      app.identity.documentId ==
          iggy3d::creative::kInvalidDocumentId &&
      app.identity.objectCount == 0U &&
      app.identity.nextObjectId ==
  ...
  return expect(launched.handled && launched.accepted,
         ...
         expect(reopenedApp.identity.saveId == launchedSaveId,
                "lifecycle reopen restores active creative save") &&
         expect(reopenedApp.identity.savePath == launchedSavePath,
                "lifecycle reopen restores active creative path") &&
         expect(reopenedApp.identity.worldId == launchedWorldId,
                "lifecycle reopen restores active creative world") &&
         expect(reopenedApp.identity.documentId == launchedDocumentId,
                "lifecycle reopen restores active creative document") &&
         expect(reopenedApp.identity.objectCount == 1U,
