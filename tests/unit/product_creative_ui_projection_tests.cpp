...
bool countsMirrorDrawList(
  ...
  const iggy3d::ProductCreativeUiProjectionReceipt& receipt =
      projection.receipt;
  const iggy3d::CreativeUiDrawList& drawList = projection.drawList;
...
bool nullInputFailsClosed() {
  ...
  return expect(projection.receipt.requested, "null requested") &&
         ...
         expect(projection.receipt.theme == iggy3d::CreativeUiThemeId::Journal,
                "null theme") &&
...
bool modelInputBuildsReadyProjection() {
  ...
  return expect(projection.receipt.requested, "model requested") &&
         ...
         expect(projection.drawList.theme == iggy3d::CreativeUiThemeId::Journal,
                "model draw theme") &&
         expect(projection.receipt.theme == iggy3d::CreativeUiThemeId::Journal,
                "model receipt theme") &&
...
bool facadeInputBuildsFromFacadeState() {
  ...
  return expect(projection.receipt.ready, "facade ready") &&
         ...
         expect(selected != nullptr &&
                    selected->text ==
                        "Selected: count=1 primary=88 visible=unknown",
                "facade selected row") &&
         countsMirrorDrawList(projection);
}
...
bool facadeInputEnablesUndoFromAppStack() {
  ...
  app.history.undoSnapshots.push_back({app.facade.document(), "test"});
  iggy3d::ProductCreativeUiProjectionRequest request;
  request.creative = &app;
  const iggy3d::ProductCreativeUiProjection projection =
      iggy3d::buildProductCreativeUiProjection(request);

  const iggy3d::CreativeUiPrimitive* undoPrimitive =
      findPrimitive(projection.drawList, "creative.row.tools.undo");
  const iggy3d::CreativeUiHitRegion* undoHit =
