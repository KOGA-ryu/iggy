...
bool rotatedBoxEmitsOrientedEdgesAndWorldBounds() {
  cr::CreativeDocument document;
  cr::CreativeDocumentCreateRequest request;
  request.kind = cr::CreativeObjectKind::Wall;
  request.transform.position = {4.0, 1.0, 4.0};
  request.transform.rotationEulerRadians.y = std::numbers::pi * 0.5;
  request.hasTransformOverride = true;
  request.bounds = {{2.0, 0.0, 3.875}, {6.0, 2.0, 4.125}};
  request.hasBoundsOverride = true;
  const cr::CreativeDocumentCreateReceipt created = document.createObject(request);
  const cr::CreativeDocumentWireframeBuildResult list =
      cr::buildCreativeDocumentWireframeList(document, makeProjectionRequest());
  const cr::CreativeDocumentWireframeSegmentBuildResult segments =
      cr::buildCreativeDocumentWireframeSegments(list.drawList);
  if (!created.accepted || list.drawList.items.size() != 1U ||
      segments.segmentList.segments.size() != 12U) {
    return expect(false, "rotated wireframe prerequisites built");
  }

  const cr::CreativeDocumentWireframeItem& item = list.drawList.items.front();
  const cr::CreativeDocumentWireframeSegment& first =
      segments.segmentList.segments.front();
  return expect(item.hasBoxCorners,
                "rotated box carries exact transformed corners") &&
         expect(nearVec3(item.bounds.min, {3.875, 0.0, 2.0}) &&
                    nearVec3(item.bounds.max, {4.125, 2.0, 6.0}),
                "rotated wireframe item publishes transformed world AABB") &&
         expect(nearVec3(first.start, {3.875, 0.0, 6.0}) &&
                    nearVec3(first.end, {3.875, 0.0, 2.0}),
                "rotated wireframe edges use oriented corners, not the AABB");
}
...
int main() {
  const bool ok = emptyDocumentReturnsStableEmptyReceipt() &&
                  ...
                  rotatedBoxEmitsOrientedEdgesAndWorldBounds() &&
