...
void testExactContent() {
  ...
  expect(q.id == "linear_one_unknown_3a_plus_5_eq_20_v1" && q.version == 1U &&
             ...
             q.description == "Linear equations · vocabulary and solving",
  ...
  constexpr std::array<std::string_view, 7U> prompts{{
      ...
      "What first move begins isolating a while keeping the equation balanced?",
  ...
  constexpr std::array<std::string_view, 7U> hints{{
      "Look for the equals sign and the highest power of a.",
      "The unknown is the symbol whose value the equation asks us to find.",
      "A coefficient is a number that multiplies an unknown.",
      "Undo the +5 and make the same change on both sides.",
      "20 minus 5 is 15, and the term 3a has not changed yet.",
      "Undo multiplication by 3 with its inverse operation.",
  ...
  for (std::size_t step = 0; step < q.steps.size(); ++step) {
    expect(q.steps[step].prompt == prompts[step] &&
               ...
               q.steps[step].wrongHint == hints[step] &&
